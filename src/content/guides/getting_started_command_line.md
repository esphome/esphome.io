...
description: >-
  Getting Started guide for installing ESPHome using the command line and
...
Follow the instructions in [Link](physical_device_connection/) to connect to your
ESP device.
...
So now is a great time to go take a look at the [Components Index](/index).
...
[Discord chat](https://discord.gg/KhAMKrd) (also make sure to read the [FAQ](faq)).
...
as a [Home Assistant add-on](getting_started_hassio), but can run in docker independently from
Home Assistant.
