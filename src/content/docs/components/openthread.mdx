...
> [!NOTE]
> `esphome.ota` does not work when `poll_period` > 0. Use `openthread.set_poll_period` to set the poll period to 0 before attempting OTA, then set it back to the original value afterwards to restore sleepy (SED) behavior.
...
Sets the poll period at runtime on an MTD device. When `poll_period` > 0 the radio is off while idle (sleepy end device); when 0 the radio is always on. This action is a no-op on FTD devices.
...
- **poll_period** (*Required*, [Time](/guides/configuration-types#config-time)): The poll period to apply. It must always be provided, including in the short form (`openthread.set_poll_period: 5s`). This is distinct from the component-level `poll_period:` option above, which is set once at boot.
