...
> [!NOTE]
> esphome.ota does not work when poll_period > 0. Use openthread.set_poll_period to set poll_period to 0 before attempting ota, then set it back to the original value afterwards to restore sleepy (SED) behavior.
...
- **poll_period** (*Required*, [Time](/guides/configuration-types#config-time)): The poll period to apply. It must always be provided, including in the short form (`openthread.set_poll_period: 5s`). This is distinct from the component-level `poll_period:` option above, which is set once at boot. On FTD devices the action is a no-op.
