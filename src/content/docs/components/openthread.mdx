...
- **tlv** (*Required on nRF52840, optional on ESP32*, string): The Thread dataset as a hex-encoded TLV string, taken
  from the Thread information in Home Assistant.
...
> [!NOTE]
> These options (`poll_period`, `output_power`, and the dataset fields below) apply only on ESP32 and are ignored on
> nRF52840. On nRF52840, configure the network with the `tlv` option instead.
