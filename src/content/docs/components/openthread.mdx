...
- The `zephyr` platform's `esp32h2`, `esp32c6`, and `nrf52` variants — see the
  [Zephyr platform's variant table](/components/zephyr/#variants). Not available on `esp32` (original) or
  `native_sim`. These build OpenThread from mainline Zephyr source (Thread 1.4) rather than Nordic's
  prebuilt libraries, regardless of variant.
...
(`esp32h2`, `esp32c6`, `nrf52`), the dataset must be provided as a hex-encoded `tlv` string from Home Assistant's
Thread integration — individual fields are not yet supported there.
