...
> [!NOTE]
> esphome.ota does not work when poll_period > 0.  Use openthread.set_poll_period to set poll_period to 0 before attempting ota.
...
- **`bool is_sed() const`**: Returns `true` when the device is operating as a Sleepy End Device (i.e. `poll_period > 0`), `false` otherwise.

```yaml
on_...:
  then:
    - lambda: |-
        if (id(openthread_id).is_sed()) {
