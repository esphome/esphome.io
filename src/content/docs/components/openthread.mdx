...
# Example Zephyr configuration for nRF52840 board
nrf52:
...
The configuration options vary by platform. On ESP32, the full dataset can be specified with individual fields. On nRF52840, only the **TLV** approach is supported — provide the dataset as a hex-encoded TLV string from Home Assistant's Thread integration.

```yaml
# Example OpenThread component configuration (ESP32)
network:
  enable_ipv6: true
...
# Example OpenThread component configuration (nRF52840)
network:
  enable_ipv6: true
...
- **device_type** (*Optional*, enum): OpenThread Device Type, either `FTD` or `MTD`. Defaults to `FTD`.
  Supported on both ESP32 and nRF52840.
- **tlv** (*Required on nRF52840; optional on ESP32*, string): dataset TLVs from the Thread information in Home Assistant.
  (the individual key/channel/PAN fields below are ESP32-only).
- **channel** (*Optional*, int): Channel number from 11 to 26. ESP32 only.
- **network_name** (*Optional*, string): A human-readable Network Name. ESP32 only.
- **network_key** (*Optional*, string): OpenThread network key. ESP32 only.
- **pan_id** (*Optional*, string): 2-byte Personal Area Network ID (PAN ID). ESP32 only.
- **ext_pan_id** (*Optional*, string): 8-byte Extended Personal Area Network ID (XPAN ID). ESP32 only.
- **pskc** (*Optional*, string): PSKc is used to authenticate an external Thread Commissioner to a Thread network. ESP32 only.
- **mesh_local_prefix** (*Optional*, ipv6network): Used to build Mesh-Local IPv6 addresses (ML-EIDs), which are unique to
  each Thread device within the network partition. ESP32 only.
...
- **poll_period** (*Optional*, [Time](/guides/configuration-types#config-time)): When Poll_Period is set on an MTD device,
  the parent router will enqueue any messages and wait for the child to submit a poll data request
...
When using nRF52840 with Zephyr, the component is built on the Nordic Semiconductor nRF Connect SDK
and includes the OpenThread stack natively. The device must be connected to a Thread network via a
[Thread border router](https://www.home-assistant.io/integrations/thread).

IPv6 networking is **required** on nRF52840 and is enabled by default. The ``network:`` component
automatically configures the Zephyr networking stack for IPv6 when an nRF52 board is selected.

The following Zephyr Kconfig options are automatically enabled:

- ``NET_L2_OPENTHREAD`` — OpenThread L2 layer
- ``OPENTHREAD_NORDIC_LIBRARY_FTD`` — Nordic FTD library
- ``OPENTHREAD_SRP_CLIENT`` — SRP client for mDNS service registration
- ``OPENTHREAD_SLAAC`` — Stateless Address Autoconfiguration

For nRF52840, only the **FTD** device type is supported and it is set automatically.
