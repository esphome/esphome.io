...
- **registers** (*Optional*): A list of registers that are responded to when acting as a server.
  ...
  - **read_lambda** (**Required**, [lambda](/automations/templates#config-lambda)):
    Lambda that returns the value of this register. Returning an empty optional (`return {};`) declines
    the read, and the client's request is answered with a ModBUS exception instead. This is useful when
    the value isn't available right now, for example while a sensor is still warming up. A plain
    `return <value>;` still works as before and does not need to change.
...
- **bits** (*Optional*): A list of bits that are responded to when acting as a server. Bits serve
  Modbus coils and discrete inputs from one shared table: a coil and a discrete input at the same
  address are the same bit, the same way holding and input registers share the register table above.
  A client can read a bit as a coil or as a discrete input and gets the same value either way. Each
  bit address must be unique within the `bits` list.
  ...
  - **read_lambda** (**Required**, [lambda](/automations/templates#config-lambda)):
    ...
    of this bit. Returning an empty optional (`return {};`) declines the read, and the client's
    request is answered with a ModBUS exception instead of a value.
  ...
  - **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
    ...
    successful, `false` otherwise, in which case a ModBUS exception code `4` will be sent to the client.
    If this is not set, the bit can be read but not written.
  ...
  > the whole request is rejected before anything is changed. Once that check passes, each
  > `write_lambda` is called in turn; if one of them returns `false`, the write stops there — the
  > bits already written stay changed, and the client's request is answered with a ModBUS exception.
...
The following code adds two bits: a read-only bit reporting a relay's state, and a read/write bit
