...
- **bits** (*Optional*): A list of bits that are responded to when acting as a server. Bits serve
  ...
  > gets. The same holds the other way round: a bits-only server with no `registers` and no
  > `courtesy_response` answers `ILLEGAL_FUNCTION` to register reads.
