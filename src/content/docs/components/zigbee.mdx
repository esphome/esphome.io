...
description and present_value attributes. With `cluster: device_class`, the occupancy cluster is used for the `occupancy` device class.
...
`cluster: device_class` is supported for `temperature`, `humidity`, `atmospheric_pressure`, `pressure`, `volume_flow_rate`,
`illuminance`, `pm25`, `carbon_dioxide` device classes.
