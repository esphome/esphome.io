...
- **cluster** (*Optional*, enum): Zigbee cluster to be used. One of `generic` or `device_class` or specific cluster
  names. Valid options depend on the component. `generic` uses basic analog/binary Zigbee clusters and works for
  all device classes. `device_class` can only be used for supported device classes and uses a Zigbee cluster fitting
  to the device class. Likewise, some components support setting specific cluster names, for example `on_off` for
  switches. Benefits are better support by Z2M/ZHA and compatibility with 3rd party Zigbee devices. Component names
  are not exposed when using `device_class` or cluster names. On `nRF52` only the basic clusters are supported.
  Defaults to `generic`.
- **endpoint** (*Optional*, int): Endpoint number in the range from 1 to 239. This defines which entity goes
  on which endpoint, giving a more stable configuration and letting you manually combine entities that logically
  belong together. Only entities that map to different Zigbee clusters can be combined (for example
  a `binary_sensor` and a `sensor`); two entities of the same type, such as two sensors, cannot share an
  endpoint unless `cluster: device_class` maps them to different clusters (for example a temperature and
  a humidity sensor). If more than one entity on an endpoint has a Zigbee device type, set
  `use_device_type: true` on one of them.
...
[Binary Sensors](/components/binary_sensor#config-binary_sensor) are implemented by default as binary input clusters with
description and present_value attributes. With `cluster: device_class`, the occupancy cluster is used for the `occupancy`
device class.


### Sensor

[Sensors](/components/sensor#config-sensor) are implemented by default as analog input clusters with description,
engineering_units, and present_value attributes. On `ESP32` additionally the application type and resolution
attributes are set if applicable. `cluster: device_class` is supported for `temperature`, `humidity`,
`atmospheric_pressure`, `pressure`, `volume_flow_rate`, `illuminance`, `pm25`, and `carbon_dioxide` device classes.
...
With `cluster: on_off`, the on_off cluster is used instead.
