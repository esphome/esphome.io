...
and `nrf54lm20a` [variants](/components/zephyr/#variants) with `framework: type: zigbee` set (see
[below](#platform-zephyr-setup)), and on `ESP32` with IEEE 802.15.4 connectivity (`ESP32-C5`, `ESP32-C6`, or
...
Set `framework: type: zigbee` on the `zephyr` component to build against Nordic's `ncs-zigbee` add-on
(this is auto-selected when `zigbee:` is present and no `framework: type:` is given explicitly).
The stock devicetree for these boards has no partitions for Zigbee's non-volatile storage, so an
...
Only on the `nRF52` chip family (`platform: nrf52`, or `platform: zephyr` with `framework: type: zigbee`):
