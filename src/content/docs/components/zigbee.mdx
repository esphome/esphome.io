...
- **cluster** (*Optional*, enum): Zigbee cluster to be used. One of `generic` or `device_class`. `generic`
  uses basic analog/binary Zigbee clusters and works for all device classes. `device_class` can only be
  used for supported device classes and uses a Zigbee cluster fitting to the device class. Benefits are
  better support by Z2M/ZHA and compatibility with 3rd party Zigbee devices. Component names are not exposed
  when using `device_class`. On `nRF52` only the basic clusters are supported. Defaults to `generic`.
