...
> When using Zigbee2MQTT make sure to use at least version 2.8.0. Otherwise names cannot have spaces in them and you need to setup at least 2 endpoints.
...
- `ESP32-H2` seems to work more reliable than `ESP32-C6`. There are reports of `ESP32-C6` boards that work only if placed next 
to the coordinator. [Here is a list with known good boards.](https://github.com/luar123/zigbee_esphome/discussions/41#discussioncomment-14978980)
- On `ESP32` Zigbee can be used together with [wifi](/components/wifi) with some limitations:
  ...
  - Wifi station mode + Zigbee router leads to a instable Zigbee network. Still can be used if wifi is only enabled for short times (e.g. OTA)
  - Wifi access point + Zigbee end device could lead to a unstable access point with increased package loss. If the AP is only used as fallback there should be no issues.
...
- **report** (**Optional**, enum): Report the state. One of `yes`, `no`, `force`. `yes` activates reporting with default settings and uses
the configuration from the coordinator if possible. `force` ignores the the coordinator settings and reports
every change. On `nrf52` always the coordinator settings are used. Defaults to `yes`.
