...
- **cluster** (*Optional*, enum): Zigbee cluster to be used. One of `generic` and `device_class`. `generic` uses basic analog/binary Zigbee
  clusters and works for all device classes. `device_class` can only be used for supported device classes and uses a Zigbee cluster
  ...
  names are not exposed when using `device_class`. On `nRF52` only `generic` is supported. Defaults to `generic`.
- **endpoint** (*Optional*, int): Endpoint number in the range from 1 to 239. This defines which entity goes on which
  ...
  two entities of the same type, such as two sensors, cannot share an endpoint unless `cluster: device_class` maps them to
...
If `cluster: device_class` the occupancy cluster is used for the `occupancy` device class.
