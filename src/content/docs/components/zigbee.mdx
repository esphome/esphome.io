...
Zigbee is only supported on the [`nrf52` platform](/components/nrf52/) (not the `zephyr` platform's `nrf52` variant,
which does not support Zigbee) and on `ESP32` with IEEE 802.15.4 connectivity (`ESP32-C5`, `ESP32-C6`, or `ESP32-H2`
