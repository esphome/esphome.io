...
> [Modbus Client](/components/modbus_client/) for ad-hoc requests, [Modbus Server](/components/modbus_server/)
> for serving registers to another device, and [Modbus Sniffer](/components/modbus_sniffer/) for listening
> to a bus other devices are talking on.
...
- **role** (*Optional*, string): The role of this component, `client`, `server` or `sniffer`. Defaults to `client`.
  `sniffer` only listens and never transmits; see [Modbus Sniffer](/components/modbus_sniffer/).
...
- For passively watching the requests and replies of a bus you don't control, use
  [Modbus Sniffer](/components/modbus_sniffer/)
