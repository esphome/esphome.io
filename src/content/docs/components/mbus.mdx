...
- **mbus_id** (_Optional_, [ID](/guides/configuration-types#id)): Manually specify the ID of the mbus master.
- **secondary_address** (_Optional_): The secondary address of the M-Bus slave.
- **interval** (_Optional_, [Time](/guides/configuration-types#time)): Interval of the command loop for reading M-Bus slaves (default: `1min`).
...
- **factor** (_Optional_): Divider for scaling the reading (default: `1.0`). The value is divided by the factor before being published to Home Assistant.
