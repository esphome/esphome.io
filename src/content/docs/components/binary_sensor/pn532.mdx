...
- **Flapping tag detection fix:** Prevents rapid alternating `on_tag`/`on_tag_removed` events when a tag is left on the reader.
- **Reliability:** Includes a health check with configurable auto-reset to recover from I2C bus freezes.
- **Interference Reduction:** The RF field is disabled between polls by default to reduce interference with 2.4GHz WiFi antennas.
- **Bus Health:** Implements exponential backoff on bus failures to prevent log flooding.
...
- **health_check_enabled** (*Optional*, boolean): Enable periodic health checks to verify communication with the PN532. Defaults to `true`.

- **health_check_interval** (*Optional*, [Time](/guides/configuration-types#time)): How often to run the health check. Defaults to `60s`.

- **max_failed_checks** (*Optional*, integer): Number of consecutive failures before marking the component as unhealthy. Defaults to `3`.

- **auto_reset_on_failure** (*Optional*, boolean): Attempt to re-initialize the PN532 if it becomes unresponsive. Defaults to `true`.

- **rf_field_enabled** (*Optional*, boolean): Keep the RF field on between polls. Setting this to `false` can reduce WiFi interference. Defaults to `false`.
...
- **address** (*Optional*, integer): The I²C address of the PN532. Defaults to `0x24`.
...
- **uid** (**Required**, string): The unique ID of the NFC/RFID tag. This is a hyphen-separated or colon-separated list
  of hexadecimal values. For example `74-10-37-94` or `74:10:37:94`.
