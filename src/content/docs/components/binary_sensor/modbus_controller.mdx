...
- **response_size** (*Optional*, int): Size of the device's response in bytes. The default is derived from `register_type`. Only override it for non-standard response lengths, including a `custom_command` whose reply is longer than one register. Has no effect on the `coil` and `discrete_input` register types, which are always read as a single bit.
