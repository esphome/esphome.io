...
<span id="modbus_controller-continuous"></span>

- **continuous** (*Optional*, boolean): Poll as fast as the bus allows instead of once per update interval. Defaults to `false`. See [Continuous polling](#modbus_controller-continuous-polling) below.
...
With `continuous: true` every read this controller makes is handed to the [Modbus](/components/modbus/) hub as a continuous poll.
The hub sends the read again as soon as the previous response has been handled, so reads stream back to back and fill time the bus would otherwise spend idle.

`update_interval` keeps its meaning as the recovery path.
A continuous poll ends when a read fails or is cancelled, and the next update re-submits it, so a device that stops answering is retried on the normal polling cadence.
Set `update_interval` to a sensible value even when polling continuously.

Writes and one-shot reads keep priority over continuous polls in the hub's transmit queue, so a busy bus does not starve commands.
The option applies to reads only: the hub strips it from function codes that change something on the device.

Continuous polling suits a device that should be read with as little delay as possible, typically one with a small set of registers.
The trade-off is that the bus is kept busy, which matters when several devices share it.
