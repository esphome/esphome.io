...
- **offline_skip_updates** (*Optional*, integer): How many update intervals to wait between retry attempts while a
  device is offline (see below). If using a bus with multiple slaves, this avoids waiting for timeouts on an
  unresponsive device, allowing the other slaves on the same bus to be read on time.
...
Multiple `modbus_controller` instances sharing the same `address` works — for example, to poll the same device at
different `update_interval`s.
