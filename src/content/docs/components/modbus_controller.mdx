...
> - **allow_duplicate_commands**: Polling commands are now always deduplicated by the `modbus` hub while an identical
>   frame is already pending. One-shot commands (writes and custom commands) are always transmitted; avoiding duplicate
