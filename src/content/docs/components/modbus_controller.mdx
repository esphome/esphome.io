...
Check out the various Modbus components available at the bottom of the document in the [.. _modbus_controller-automations:](#modbus_controller-automations) section. They can be directly defined *(inline)* under the `modbus_controller` hub or as standalone components. Technically there is no difference between the *inline* and the standard definitions approach.
...
> [!NOTE]
> Write support is only implemented for numbers, switches and selects; If you want ad-hoc write support see [Modbus Client](/components/modbus_client).
