...
- **address** (*Optional*, int): The Modbus device address (can be decimal or hexadecimal). The valid Modbus device
  range is `1`-`247`. `0` is the broadcast address; broadcasts are not supported by `modbus_controller` because no
  response is ever returned. To send broadcast (or other ad-hoc) frames, use the
  [Modbus Client](/components/modbus_client/) component instead. Defaults to `0x01`.
