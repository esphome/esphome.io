...
resumes after it. `continuous` applies to reads only - the hub strips it from any function code that changes
...
and the next update re-submits it, so a device that stops answering is retried on the normal polling cadence. Set
`update_interval` to a sensible value even when polling continuously.
