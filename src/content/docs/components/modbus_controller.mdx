...
- **address** (**Required**, [ID](/guides/configuration-types#id)): The Modbus address of the slave device.
  It is now possible to have two `modbus_controller` instances with the same `address` in one configuration, for
  example if each should poll the same device at a different `update_interval`.
...
> [!WARNING]
> Several options have been removed and now raise a validation error:
>
> - **command_throttle**: Use [`turnaround_time`](/components/modbus/) on the `modbus` component instead.
> - **allow_duplicate_commands**: Command queuing (including de-duplication) is now handled in the `modbus` component.
...
The value of `custom_pdu` is a PDU: the modbus function code followed by its data. Do **not** include the leading
device address byte — the controller's `address` is used automatically and the CRC is appended for you.

> [!WARNING]
> `custom_command` has been renamed to `custom_pdu`. A configuration still using `custom_command` now fails validation
> with an error asking you to rename it to `custom_pdu` and drop the leading device address byte. As a result, a custom
> command can no longer target a different address than its controller.
...
- platform: modbus_controller
  ...
  #    address: 0x156
  #    register_type: "read"
  ## reimplement using custom_pdu (no device address byte)
...
>
> To send a command, hold a `std::optional<ModbusCommandItem>` and call `.send()` on it, as shown below.
> `ModbusController::queue_command()` is deprecated (from 2026.8.0, removed in 2027.2.0); replace calls such as
> `controller->queue_command(command)` with a `command.emplace(...); command->send();` pattern.
>
...
>             static std::optional<esphome::modbus_controller::ModbusCommandItem> set_rtc_command;
>             set_rtc_command.emplace(
>                 esphome::modbus_controller::ModbusCommandItem::create_write_multiple_command(controller, 0x9013, 3, rtc_data));
>             // Submit the command
>             set_rtc_command->send();
...
>           };
>           static std::optional<esphome::modbus_controller::ModbusCommandItem> set_battery1_command;
>           set_battery1_command.emplace(
>               esphome::modbus_controller::ModbusCommandItem::create_write_multiple_command(controller, 0x9000, battery_settings1.size(),
>                                                                                           battery_settings1));
>
>           static std::optional<esphome::modbus_controller::ModbusCommandItem> set_battery2_command;
>           set_battery2_command.emplace(
>               esphome::modbus_controller::ModbusCommandItem::create_write_multiple_command(controller, 0x906B, battery_settings2.size(),
>                                                                                           battery_settings2));
>           delay(200) ;
>           set_battery1_command->send();
>           delay(200) ;
>           set_battery2_command->send();
