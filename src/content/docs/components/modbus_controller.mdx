...
> The code synchronizes the local time of the MCU to the epever device.
...
>           auto *client = id(epever_client);
>           // if time is not yet synced, skip RTC update
>           if (time.is_valid()) {
>             // create the payload
>             uint16_t[] rtc_data = {uint16_t((minutes << 8) | seconds), uint16_t((day << 8) | hour),
>                                               uint16_t((year << 8) | month)};
>             // Send an ad-hoc modbus command
>             client->write_multiple_registers(0x9013, rtc_data);
>             ESP_LOGI("ModbusLambda", "EPSOLAR RTC set to %02d:%02d:%02d %02d.%02d.%04d", hour, minutes, seconds, day, month,
>                     year + 2000);
>           }
>           // Battery settings
>           // Note: these values are examples only and apply my AGM Battery
>           uint16_t[] battery_settings1 = {
...
>           uint16_t[] battery_settings2 = {
>               0x0000,  // 906B Equalize Duration (min.) 0
>               0x0075   // 906C Boost Duration (aka absorb) 117 mins
>           };
>           client->write_multiple_registers(0x9000, battery_settings1);
>           client->write_multiple_registers(0x906B, battery_settings2);
...
> 
> # This is just a hook for the lambdas above
> modbus_client:
>   - id: epever_client
>     address: 0x1
>     modbus_id: mod_bus_epever
