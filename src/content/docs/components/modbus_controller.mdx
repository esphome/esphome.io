...
- **address** (*Optional*, int): The Modbus device address (can be decimal or hexadecimal). `1`-`247` is the standard
  ...
  [Modbus Client](/components/modbus_client/) component for broadcast or other ad-hoc frames. Values above `247` are
  still accepted but fall outside the standard range. Defaults to `0x01`.
