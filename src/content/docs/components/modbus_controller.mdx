...
Non-responses are counted per device, not per command or register. After `max_cmd_retries` consecutive timeouts the
whole device is marked offline (triggering [`on_offline`](#modbus_controller-on_offline)), its queued frames are
dropped, and all of its updates are skipped. Every `offline_skip_updates + 1` update intervals the controller sends
a single retry attempt (with the normal retry ladder). Any response, or a Modbus exception response, brings the
...
> [!WARNING]
> These options no longer have any effect. They are still accepted so existing configurations keep working, and will
> be removed in 2027.2.0:
>
> - **command_throttle**: Use [`turnaround_time`](/components/modbus/) on the `modbus` component instead.
> - **allow_duplicate_commands**: Polling commands are deduplicated by the `modbus` hub while an identical frame is
>   already pending. One-shot commands (writes and custom commands) are always transmitted; avoiding duplicate
>   one-shot commands is the caller's responsibility.
>
> These options have been removed and raise a validation error:
>
