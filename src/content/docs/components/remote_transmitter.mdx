...
The `remote_transmitter` component can act as a backend for the `radio_frequency` component, allowing you to use it in Home Assistant to control RF devices.

When using an external RF module like the [CC1101](/components/cc1101/), ensure you configure its component separately and use the `on_transmit` and `on_complete` automations to switch the transceiver between transmission and reception modes.
