...
- **command** (**Required**, enum, [templatable](/automations/templates)): The Hob2Hood command to send,
  see dumper output for more info.
  - `light_off`
  - `light_on`
  - `fan_off`
...
This [action](/automations/actions#all-actions) sends an infrared remote command for Pioneer Diamante Pro mini split heat pumps, the WYT model family, to a remote transmitter. It supports sending a raw code list or specifying individual friendly fields.

```yaml
on_...:
  # Sending via raw code list:
  - remote_transmitter.transmit_pioneer_wyt:
      code: [0x23, 0xCB, 0x26, 0x01, 0x00, 0x24, 0x03, 0x05, 0x00, 0x00, 0x00, 0x00, 0x80]
...
- **code** (*Optional*, list of 13 or 14 bytes, [templatable](/automations/templates)): The raw payload. The checksum is always recalculated and stored as the 14th byte; when 13 bytes are provided, it is appended automatically. Cannot be combined with any of the friendly fields below.
...
- **target_temperature** (*Optional*, float, [templatable](/automations/templates)): The target temperature in °C, from `16.0` to `31.0` in steps of `0.5`. Defaults to `24.0`.
...
- **remote_temp** (*Optional*, int, [templatable](/automations/templates)): Current room temperature feedback for Follow Me mode, from `0` to `255`. Defaults to `0`.
