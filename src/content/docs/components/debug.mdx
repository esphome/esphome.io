...
If [`zephyr_pm`](/components/power_management/zephyr_pm/)'s **stats** option is enabled elsewhere in the
config, this component periodically logs each power state's entry count and cycles spent, at debug log
level. `zephyr_pm`'s **stats** option alone only turns the underlying counters on -- `debug` must be
present to actually see them.
