...
or ESPHome will fail in the config validation stage. If the [Ethernet](/components/ethernet/) component is used,
you can't use this component to connect to an existing WiFi network (STA), but you can set up an access point (AP)
that will use the Ethernet connection to reach the internet.
...
```yaml
# Set up a WiFi access point that uses an existing Ethernet connection
wifi:
  ap:
    ssid: mywifiap
    password: mypassword
...
ethernet:
  # See the Ethernet component for the full configuration
  type: ...
```
