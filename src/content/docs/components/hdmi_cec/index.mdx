...
hdmi_cec:
  ...
  # NOTE: In the code current revision, this decoding facility is temporarily removed
  decode_messages: false # Optional. For now, setting to 'true' has no effect.
...
chip can be used. Although the 3.3 volt level is already fine, an interface chip like a TXS0102 or TCA9517 is designed
to withstand a bus line voltage even when the local power is off, and its `i2c` pull-down behavior is compatible
with the cec bus protocol. Also, tiny PCB modules are easily available for i2c level conversion, which are fine as well.
...
**Note**: Maybe, the diode on the UART output can be replaced by a plain wire for some of the newer microcontrollers,
such as an `esp32s3` or an `rp2350`, thanks to their flexible GPIO matrix:
*After* the UART driver has done its standard pin configuration,
the pin mode can probably be overwritten to be `open drain`. This needs further testing.
