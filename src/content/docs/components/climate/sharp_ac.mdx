...
- **connection_status** (*Optional*): Optional connection state text sensor.
  ...
  - During startup this reports `Connecting (x/8)` while the component completes
    the eight-step protocol handshake, then switches to `Connected` once the
...
- The startup counter in `connection_status` covers the full handshake:
  `INIT_MSG2`, `SUBSCRIBE_MSG`, ACK, `SUBSCRIBE_MSG2`, `GET_STATE`, `GET_STATUS`,
  `CONNECTED_MSG`, and the final ACK.
