...
used to manually tune the PID controller. Make sure to update the values you want in
...
used to manually tune the PID controller deadband parameters. Make sure to update the values you want in
the YAML file! They will reset on the next reboot.
...
- **id** (**Required**, [ID](/guides/configuration-types#id)): ID of the PID Climate to update.
- **kp_multiplier** (**Required**, float, [templatable](/automations/templates)): Deadband multiplier for the
  proportional term.
- **ki_multiplier** (*Optional*, float, [templatable](/automations/templates)): Deadband multiplier for the
  integral term. If omitted, the current multiplier is kept.

- **kd_multiplier** (*Optional*, float, [templatable](/automations/templates)): Deadband multiplier for the
  derivative term. If omitted, the current multiplier is kept.
...
used to manually tune the deadband zone. Make sure to update the values you want in
the YAML file! They will reset on the next reboot.
...
- **id** (**Required**, [ID](/guides/configuration-types#id)): ID of the PID Climate to update.
- **threshold_high** (**Required**, temperature delta, [templatable](/automations/templates)): Upper deadband threshold
  relative to the target temperature.
- **threshold_low** (**Required**, temperature delta, [templatable](/automations/templates)): Lower deadband threshold
  relative to the target temperature. It must be less than or equal to `threshold_high`.

This action updates the active runtime PID controller. The new values are used on the
next PID computation cycle. If `threshold_low` is greater than `threshold_high`,
the action logs a warning and keeps the existing thresholds.
...
- **type** (**Required**, string): The value to monitor. One of
  ...
  - `DEADBAND_THRESHOLD_HIGH` - The current upper deadband threshold in °C.
  - `DEADBAND_THRESHOLD_LOW` - The current lower deadband threshold in °C.
  - `KP_DEADBAND_MULTIPLIER` - The current unitless deadband multiplier for the proportional term.
  - `KI_DEADBAND_MULTIPLIER` - The current unitless deadband multiplier for the integral term.
  - `KD_DEADBAND_MULTIPLIER` - The current unitless deadband multiplier for the derivative term.

  Deadband multipliers are published as raw factors: for example, `0.05` means 5% of the
  corresponding PID term within the deadband.
