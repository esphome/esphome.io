...
- Target temperature range: `16 °C` to `31 °C`, or `61 °F` to `88 °F` with `use_fahrenheit`
...
- **use_fahrenheit** (*Optional*, boolean): Enable Mitsubishi-specific temperature mapping for indoor units natively
  configured in Fahrenheit. This requires temperature encoding B; disable this option if the unit reports encoding A.
  Defaults to `false`.
