...
- **vane** (*Optional*): Vane state automation configuration.
  - **on_state** (*Optional*, [Automation](/automations/)): An automation to run whenever the hub publishes its current
    ...
    [Vane State Trigger](#vane-state-trigger) for the available values and an example.
...
The `vane.on_state` trigger runs whenever the Mitsubishi CN105 hub publishes its current state. This includes updates
