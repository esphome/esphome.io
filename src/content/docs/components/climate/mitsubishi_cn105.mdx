...
- **supported_swing_modes** (*Optional*, enum): Selects which swing modes are exposed by the climate entity. Defaults to `'OFF'`.
  ...
  For example, set `supported_swing_modes: BOTH` to expose all swing modes.

#### Vane controls

Vane controls are optional and can be enabled independently of the climate entity's `supported_swing_modes`. They expose Mitsubishi-specific vane options that are available on the indoor unit but are not represented by Home Assistant's generic climate model.

- **vane.vertical.direction** (*Optional*, [Select](/components/select)): Exposes the vertical vane direction as a select entity.
...
> [!NOTE]
> This does not modify the temperature reported by the indoor unit in its status/room temperature packets.
...
>
> It is the user’s responsibility to tailor the update strategy and usage to their specific indoor unit and setup.
>
