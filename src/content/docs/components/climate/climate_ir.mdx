...
- **supports_auto** (*Optional*, boolean): Enables setting auto (heat/cool) mode for this climate device. When not
  set, it is enabled automatically only if both `supports_cool` and `supports_heat` are enabled. Set it to `true` to
  advertise auto mode regardless (e.g. cool-only units that still expose an auto mode), or to `false` to hide it even
  on units that support both heating and cooling but have no usable auto mode.
