...
All GREE models can receive commands from the original remote and update the climate state. The receiver validates the
GREE transport and checksum, then decodes power, operating mode, target temperature from 16 to 30 °C, fan speed, and
the swing axes supported by the selected model. Timer and other auxiliary fields do not change the climate state.

The existing `generic`, `yan`, `yaa`, `yac`, `yac1fb9`, and `yag` models use the common GREE state fields while
allowing OEM-specific values in timer, display-temperature, I-Feel, WiFi, vane-position, and otherwise unused fields.
This is useful for GREE-compatible units sold under other brands such as Tadiran or Tosot.

The `yb1fa` and `yx1ff` models have stricter model-specific validation because their captured frame formats and
feature interactions are known. Both expose only vertical swing control. The `yb1fa` model maps the three numbered
fan speeds to low, medium, and high. The `yx1ff` model maps them to quiet, low, and medium, uses the Turbo bit for high,
and also decodes the sleep preset.
...
For models with supported GREE feature switches, received feature bits update the configured switches and are preserved
when a later climate or feature change transmits a complete GREE state frame. The existing startup restore behavior of
the legacy `yan`, `yaa`, `yac`, and `yac1fb9` switches is unchanged.

On `yb1fa` and `yx1ff`, feature switches do not restore a saved state or transmit IR during startup. They initially
publish the protocol default state and are synchronized by the first valid frame received from the physical remote.
