...
| [Arduino-HeatpumpIR](#heatpumpir) library | `heatpumpir`                       | [ZJ](#heatpumpir) |
...
This platform exposes **ECO** and **BOOST** climate presets (where supported by the AC unit). These map to the library's `FAN_5` (low-power) and `FAN_4` (turbo) fan speeds respectively. Note that not all protocols in the library support these presets &mdash; refer to the [Arduino-HeatpumpIR repository](https://github.com/ToniA/arduino-heatpumpir) for per-model compatibility.
