...
| Emmeti | `emmeti`                           | yes |
| [Fujitsu General](#fujitsu_general) | `fujitsu_general`                  | yes |
| [GREE](#gree_ir) | `gree`                             | |
...
<span id="fujitsu_general"></span>

### `fujitsu_general`

The `fujitsu_general` climate supports the modern Fujitsu IR protocol. Older units, such as the ones using the AR-JXX remotes are not supported yet.

After configuring a climate component, additional stateless commands can be sent through a button:

- **fujitsu_general_id** (**Required**, [ID](/guides/configuration-types#id)): The ID of the `fujitsu_general` climate component.
- **step_horizontal** (*Optional*, [Switch](/components/button#config-button)): Steps the air vanes to control horizontal airflow.
- **step_vertical** (*Optional*, [Switch](/components/button#config-button)): Steps the air vanes to control vertical airflow.
- **economy** (*Optional*, [Switch](/components/button#config-button)): Toggles economy mode.

```yaml
# Example configuration for buttons
button:
  - platform: fujitsu_general
    fujitsu_general_id: heat_pump_climate
    step_horizontal:
      name: "Step Horizontal"
    step_vertical:
      name: "Step Vertical"
    economy:
      name: "Toggle Economy Mode"
```
