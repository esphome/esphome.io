...
The `mitsubishi_heavy_zmp` protocol supports an optional IR receiver to keep the climate state in sync when the physical remote control is used. To enable this, set up a [Remote Receiver](/components/remote_receiver/) and pass its ID via `receiver_id`:
