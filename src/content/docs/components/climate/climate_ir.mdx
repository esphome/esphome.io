...
| [GREE](#gree_ir) | `gree`                             | yes (`yx1ff` only) |
...
The `yx1ff` model can also receive commands from the original remote and update the climate state. It decodes power,
operating mode, target temperature from 16 to 30 °C, all fan speeds, vertical swing on/off, and the sleep preset.
YX1FF only exposes vertical swing control, so horizontal and combined swing modes are not offered for this model.
