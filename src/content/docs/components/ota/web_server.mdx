...
HTTP Basic `Authorization` header (`requests.auth.HTTPBasicAuth`). The device's
default `WWW-Authenticate` challenge is also Basic, so the credentials cross the
network in cleartext. The `--ota-platform web_server` path does **not** use the
