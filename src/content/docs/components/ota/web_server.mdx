...
- The native API OTA port is blocked by a firewall while HTTP (port 80) is reachable.
...
challenge-response auth with hashed nonces (the password never crosses the wire) and
also compresses the firmware payload, while the `web_server` path relies on HTTP Basic
auth. Pass `--ota-platform web_server` to force the HTTP path anyway:

```bash
# Auto-select web_server OTA when it's the only platform configured:
esphome upload device.yaml

# Force the web_server OTA path even if `platform: esphome` is also configured:
esphome run device.yaml --ota-platform web_server
```

If a `web_server` `auth` block is configured, the CLI sends those credentials as HTTP
Basic auth automatically. The `--ota-platform web_server` path does **not** use the
[ESPHome OTA password](/components/ota/esphome/) at all; it only uses the `web_server`
`auth` credentials.
