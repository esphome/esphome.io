...
- **swap_method** (*Optional*, string): `zephyr` platform only. The MCUboot image-swap algorithm to use for
  applying an update. One of `scratch`, `move`, `offset`. Defaults to `scratch` on `esp32`/`esp32h2`/
  `esp32c6`; `nrf52`, `rp2040`, and `rp2350` default to `offset`; `nrf54l15`, `nrf54lm20a`, and
  `efr32mg24` default to `move` (pending `offset` testing on those boards, after which the default may
  change to match `nrf52`). Every `esp32`/`esp32h2`/`esp32c6` variant only supports `scratch` and
  `move` -- `offset` is not available on any Zephyr esp32-family variant. `nrf52`, `nrf54l15`,
  `nrf54lm20a`, `efr32mg24`, `rp2040`, and `rp2350` are the opposite: they only support `move` and
  `offset` -- none of their boards defines a `scratch` partition. Not applicable to the `native_sim`
  variant, which has no real bootloader.
