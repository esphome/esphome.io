...
- **swap_method** (*Optional*, string): `zephyr` platform only. The MCUboot image-swap algorithm to use for
  applying an update. One of `scratch`, `move`, `offset`. Defaults to `scratch`, except on the `nrf52`
  variant, which defaults to `offset`. Every `esp32`/`esp32h2`/`esp32c6` variant only supports `scratch`
  and `move`; `offset` is not available on any Zephyr esp32-family variant. The `nrf52` variant is the
  opposite: it only supports `move` and `offset` -- neither of its boards defines a `scratch` partition.
  Not applicable to the `native_sim` variant, which has no real bootloader.
