...
- **expander_interrupt_pin** (*Optional*, [Pin Schema](/guides/configuration-types#pin-schema)): 
Use expander_interrupt_pin config rather than interrupt_pin if the touchscreen interrupt line runs through an IO Expander rather than directly to an esp GPIO.  
Allows for configuration of touchscreen at boot but requires touch detection polling. Both interrupt_pin and expander_interrupt_pin cannot be configured at the same time and build will error out if attempted.
