...
- **intcfg_pin** (*Optional*, [Pin Schema](/guides/configuration-types#pin-schema)): Use this pin config rather than 
interrupt_pin if the touchscreen interrupt line runs through an IO Expander rather than directly to an esp GPIO.  
Allows for configuration of touchscreen at boot but requires touch detection polling.
