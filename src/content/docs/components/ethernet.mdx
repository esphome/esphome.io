...
The [WiFi](/components/wifi/) component may also be configured in access point (AP) mode alongside Ethernet, so that
clients connecting to the AP reach the network through the Ethernet connection.
