...
This component cannot be used together with the [WiFi](/components/wifi/) component in station (STA) mode. You can,
however, configure the WiFi component in access point (AP) mode so that clients connecting to the AP reach the network
...
```yaml
# Example: run a WiFi access point alongside Ethernet
wifi:
  ap:
    ssid: myap
    password: mypassword
