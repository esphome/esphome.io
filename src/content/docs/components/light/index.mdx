...
- **restore_state** (*Optional*): Persist the light's state to flash and restore it on bootup, with optional
  ...
  - **state** (*Optional*, string or boolean): `KEEP` (default, case-insensitive) to restore the saved ON/OFF
    state as-is, `INVERT` to restore it inverted, `INITIAL` to use **initial_state**'s state instead
    or an explicit `ON`/`OFF` (or boolean) to always force this value regardless of what was saved.
...
- **restore_mode** (*Optional*): Control how the light attempts to restore state on bootup. Mutually exclusive
  ...
  To migrate from `restore_mode` to `restore_state` these are the equivalent configurations:
  ...
  | `ALWAYS_OFF` | *(neither `restore_mode` nor `restore_state` set)* | — |
