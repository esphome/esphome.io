...
- **power_estimation** (*Optional*): Configure power estimation and optional automatic current limiting for
  the LED strip. When configured, ESPHome estimates the total current draw from the per-channel mA constants
  and the current pixel buffer values. See [Power Estimation](#addressable-power-estimation) for details.
