...
- **resume_effect** (*Optional*, boolean): Remember the effect that was running when the light was turned off,
  and start it again when the light is next turned on with a plain turn-on. A turn-on that also sets an effect,
  brightness, color or transition uses those instead. The remembered effect is kept in memory only, so it is lost
  on reboot. Defaults to `false`.
...
- **color** (*Optional*, string or hex value): A
  ...
  **red**, **green** and **blue**. Names are not case-sensitive.
  ...
  `0x223344`, the **color_brightness** is therefore set to match (about 55% and 27% respectively).
  Colors with a channel at full level, such as `red`, set **color_brightness** to 100%. If you also
  set **color_brightness**, your value is used instead, and a warning is logged if the color is a dark one.
...
- **color** (*Optional*, string or hex value): A
  [standard CSS color name](https://developer.mozilla.org/en-US/docs/Web/CSS/named-color)
  or a hex value in the form `0xRRGGBB` to use instead of **red**, **green** and **blue**. Not templatable.
  Also sets **color_brightness**, as described in [light state](#light-state).
