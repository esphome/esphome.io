...
- **rgb_order** (*Optional*, string): The RGB order of the strip. Exactly one of `rgb_order` or `rgbw_order` must be
  set.
...
- **rgbw_order** (*Optional*, string): The RGBW channel order of the strip, given as any permutation of the letters
  `R`, `G`, `B` and `W` — for example `GRBW` or `WGRB`. Exactly one of `rgb_order` or `rgbw_order` must be set.
  Cannot be combined with `is_rgbw` or `is_wrgb`.
