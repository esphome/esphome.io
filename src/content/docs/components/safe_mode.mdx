...
- **mark_boot_ok** (*Optional*, [Action](/automations/actions)): An action to mark the boot as successful, preventing
   safe mode from being invoked. This is primarily useful in conjunction with devices that boot, take a sensor reading,
   wait for the sensor data to be sent and then sleep. Rather than waiting for the `boot_is_good_after` time to elapse,
   you can mark the boot as successful. Without this your device could potentially do an OTA rollback even though the
   new firmware is functional.
