...
- **esp8266_store_log_strings_in_flash** (*Optional*, boolean): **ESP8266 only, deprecated.**
   Log format strings always live in flash via `PROGMEM`; the logger reads and formats them from
   there when a line is logged. Setting this to `false` used to copy every format string into the
   ESP8266's limited RAM, which made logging no faster and could cause out-of-memory crashes; it is
   now ignored, logs a warning, and will be rejected from 2027.4.0. Remove the option from your
   configuration.
