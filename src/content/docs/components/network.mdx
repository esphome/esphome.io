...
- **enable_ipv4** (*Optional*, boolean): Enables IPv4 support. Defaults to `true`.  Can only be `false` when the following conponents are __not__ configured: `esp32_improv`, `ethernet`, `modem`, `mqtt`, `udp`, `wifi`, or `wireguard`.
