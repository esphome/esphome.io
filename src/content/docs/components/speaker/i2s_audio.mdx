...
- **keep_alive** (*Optional*, boolean): If `true`, the I²S channel is kept allocated between audio sessions instead of being fully torn down. This prevents the audible pop/click caused by the I²S peripheral reinitializing on every audio event. A pre-warm step at boot moves the one-time initialization transient to startup. Defaults to `false`.
