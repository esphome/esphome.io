...
- **allowed_origins** (*Optional*, list of strings): A list of origins that browsers are allowed to make
  ...
  entry allows any origin, but this is not recommended.
...
boundary. To restrict access itself, use `auth:` and keep devices on a trusted, segmented network. See the
[Security Best Practices](/guides/security_best_practices/) guide.
