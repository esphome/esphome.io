...
through any browser and a simple [REST API](/web-api#api-rest). The component still uses a good amount of
flash, though significantly less than in older releases, and it has been heavily optimized to be much
cheaper to include, including on the ESP8266; each connected client uses some memory, so check your
device's free heap if you combine it with many other features on a memory-constrained board.
