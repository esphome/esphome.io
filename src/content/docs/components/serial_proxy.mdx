...
- **port_type** (**Required**, string): The electrical type of the serial port. One of:
  ...
  - `USB_SERIAL`: A serial device attached through a USB bridge. Requires `uart_id` to refer to a
    [USB UART](/components/usb_uart/) channel.
...
- **Identify USB devices** — if `uart_id` refers to a [USB UART](/components/usb_uart/) channel, the API client can
  read the identity of the attached USB device and is notified when a device is attached or removed.
