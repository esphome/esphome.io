...
The Lottie widget renders a Lottie JSON animation through LVGL's ThorVG/vector support. Animations may be embedded into the firmware from a local file or loaded from a filesystem path at runtime.

**Configuration variables:**

- **file** (*Optional*, string): Local Lottie JSON file to embed into the firmware. The widget can auto-detect its size from the JSON unless `width` and `height` are specified.
- **src** (*Optional*, string): Absolute filesystem path to a Lottie JSON file on the device, such as `/sdcard/animation.json`. When `src` is used, `width` and `height` are required.
- **width** (*Optional*, [size](/components/lvgl#lvgl-size)): Render width. Required with `src`; optional with `file`.
- **height** (*Optional*, [size](/components/lvgl#lvgl-size)): Render height. Required with `src`; optional with `file`.
...
- **width** (*Optional*, [size](/components/lvgl#lvgl-size)): Render width. Required with `src`; optional with `file` if the SVG size can be detected.
- **height** (*Optional*, [size](/components/lvgl#lvgl-size)): Render height. Required with `src`; optional with `file` if the SVG size can be detected.
- Style options from [Style properties](/components/lvgl#lvgl-styling).
