...
The chart widget plots one or more sensors as a line or bar chart over time. Each series is pushed a new point either
whenever its sensor publishes a new state, or, if `update_interval` is set, at a fixed interval read from the
sensor's current state.

**Configuration variables:**

- **series** (**Required**, list): The sensors to plot.
  - **sensor** (**Required**, [ID](/guides/configuration-types#id)): The sensor providing the series' values.
  - **color** (*Optional*, [Color](/components/lvgl#lvgl-styling)): The color of the series' line or bars.
...
- **type** (*Optional*, string): The chart type, either `LINE` or `BAR`. Defaults to `LINE`.
- **point_count** (*Optional*, int): The number of points kept per series. Each point occupies 4 bytes of RAM per
  series (plus 4 bytes of flash per series when `persist` is enabled), so large values increase memory usage.
  Defaults to `100`.
- **decimals** (*Optional*, 0-4): The number of decimal places kept when storing values, which also sets the
  resolution of the automatic Y-axis range. Defaults to `1`.
- **min_value** / **max_value** (*Optional*, float): Fixed bounds for the Y-axis. If either is set, both must be
  ...
  example, values between 12.7 and 21.3 give a range of 12 to 22).
...
- **point_radius** (*Optional*, int): The radius of the point markers, shared by all series on the chart (LVGL does
  not support a separate marker size per series).
- **persist** (*Optional*, boolean): ESP32 only. Saves the series history to flash so it survives a reboot. Defaults
  to `false`.
