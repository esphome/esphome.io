...
> [!NOTE]
> `RGB565` and `RGB` images are supported, including alpha transparency. File images and animations can use
> [automatic image encoding](/components/image/file/#automatic-image-encoding) to select their format at build time.
> See [Images](/components/image/) for loading images into ESPHome.
> [!TIP]
