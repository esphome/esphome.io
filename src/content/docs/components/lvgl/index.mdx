...
- **debug_borders** (*Optional*, boolean): When `true`, every widget is drawn with a 1 pixel border, each in a different
  color, to make the size and position of widgets easier to see while designing a layout. Colors repeat after 12 widgets.
  The border replaces any border set on the widget itself, and it takes up space, so widgets and their content may move
  by about 1 pixel. Tab and tile pages, message box frames and other parts that LVGL creates internally are not
  outlined. Intended for design use only. When more than one LVGL configuration is used, the value must be the same in
  all of them. Defaults to `false`.
