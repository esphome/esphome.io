...
- **color_depth** (*Optional*, int): The output color depth: `16` (RGB565, two bytes per pixel, default) or `24`
  (RGB888, three bytes per pixel). All LVGL configurations in one firmware must use the same depth.
  See [RGB888 Output](#rgb888-output) for display requirements.
...
Set `color_depth: 24` to preserve eight bits per red, green and blue channel. The display driver must declare a
native 24-bit path through its framebuffer and panel interface. A driver that accepts RGB888 input but converts it
to RGB565 does not qualify. Configuration validation rejects unsupported or unknown paths before compilation.

Supported paths are:

- [MIPI DSI](/components/display/mipi_dsi/) with both `color_depth: 24` and `pixel_mode: 24bit`.
- [Snapshot Display](/components/display/snapshot/) with `color_depth: 24` on the host platform.

Configure a supported display with ID `my_display` and native 24-bit output, then select it in LVGL:
...
RGB888 requires `little_endian` output, which is selected automatically from these drivers' metadata. Its packed
memory layout is B, G, R. An explicit `big_endian` setting is rejected. The panel's RGB/BGR color-order setting is
separate from this byte order. Existing 16-bit configurations retain their defaults.

RGB888 buffers and display transfers use 50% more bytes than RGB565. For example, an 800x800 framebuffer grows from
1,280,000 to 1,920,000 bytes. A software rotation buffer also uses three bytes per pixel. Check available memory,
pixel clock and panel timing limits on the device; RGB888 does not promise a higher frame rate. Reduce `buffer_size`
if an LVGL draw-buffer allocation fails, or use 16-bit output if the display path cannot sustain 24-bit transfers.

Image storage is configured separately from LVGL output. Use
[automatic image encoding](/components/image/file/#automatic-image-encoding) to embed file images and animations
in the format selected for their target. Transparent images still require blending at runtime.
...
For RGB888, use three bytes per pixel in this calculation. Buffers are rounded to whole rows.
