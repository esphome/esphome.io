...
- **color_depth** (*Optional*, string): The color depth at which the contents are generated. Supported values are `16` (RGB565, 2 bytes/pixel) and `32` (RGB888, 3 bytes/pixel). Defaults to `16`, except for MIPI DSI displays where RGB888 may be selected automatically when supported by the display path.
- **buffer_size** (*Optional*, percentage): The percentage of screen size to allocate buffer memory. If unconfigured, the default is `100%` with runtime fallback to `12%` if a full size buffer allocation fails. For devices without PSRAM, the recommended value is `25%`. Smaller configured values are rounded to the closest internal draw-buffer fraction.
...
- **fps_benchmark** (*Optional*, boolean): Enable the ESPHome LVGL FPS benchmark helper for diagnostics. Defaults to `false`.
- **perf_monitor** (*Optional*, boolean): Enable LVGL's built-in performance monitor and ESPHome idle-time wrappers. Defaults to `false`.
- **profiler** (*Optional*, boolean): Enable LVGL's built-in profiler support for development diagnostics. Defaults to `false`.
