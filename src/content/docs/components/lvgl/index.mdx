...
- **color_depth** (*Optional*, string): The color depth at which the contents are generated. Supported values are `16` (RGB565, 2 bytes/pixel) and `32` (RGB888, 3 bytes/pixel). Defaults to `16`, except for MIPI DSI displays where RGB888 may be selected automatically when supported by the display path. On ESP32-P4 MIPI DSI displays, RGB888 is usually the fastest/default path; RGB565 remains useful when reducing memory use is more important than color depth.
- **buffer_size** (*Optional*, percentage): The percentage of screen size to allocate buffer memory. If unconfigured, the default is `100%` with runtime fallback to `12%` if a full size buffer allocation fails. For devices without PSRAM, the recommended value is `25%`. Smaller configured values are rounded to the closest internal draw-buffer fraction, which allows memory-constrained or performance-tuned configurations to request smaller partial draw buffers instead of being rounded up to one eighth of the screen.
- **direct_mode** (*Optional*, boolean): Enable LVGL direct rendering mode when the display driver can present full frame buffers directly. Defaults to `false`.
- **use_ppa** (*Optional*, boolean): Enable ESP32-P4 PPA acceleration for supported LVGL draw operations. Defaults to `false`.
- **use_ppa_img** (*Optional*, boolean): Enable ESP32-P4 PPA acceleration for supported image draw operations. This also enables `use_ppa`. Defaults to `false`.
- **fps_benchmark** (*Optional*, boolean): Enable ESPHome-side LVGL frame/FPS diagnostics. This writes periodic timing information to the device logs, including frame rate and draw/flush timing, and is intended for short diagnostic runs. Defaults to `false`.
- **perf_monitor** (*Optional*, boolean): Enable LVGL's built-in on-display performance monitor and ESPHome idle-time wrappers. The monitor is drawn by LVGL on the display, so it can slightly affect the workload being measured. Defaults to `false`.
- **profiler** (*Optional*, boolean): Compile LVGL's built-in profiler support for development diagnostics. This increases firmware size/RAM use and is meant for targeted debug builds rather than normal runtime use. Defaults to `false`.
...
Internally, configured percentages are mapped to supported draw-buffer fractions such as `100%`, `50%`, `25%`, `12.5%`, `6.25%`, `3.125%` and `1.5625%`. This allows memory-constrained or performance-tuned ESP32 display configurations to request smaller partial draw buffers instead of being rounded up to `12.5%` / 1/8 of the screen.
