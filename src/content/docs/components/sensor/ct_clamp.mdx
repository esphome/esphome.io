...
voltage sensor (like the [ADC sensor](/components/adc/)) and convert the readings to measured single phase AC current.

First, you need to set up a voltage sensor source ([ADC sensor](/components/adc/), but for example also
[ADS1115](/components/ads1115/)) and pass it to the CT clamp sensor with the `sensor` option.
