---
description: "Instructions for setting up Xiaomi S200 and S400 bluetooth-based scales in ESPHome."
...
Composition Scale S400 using the [ESP32 BLE Tracker](/components/esp32_ble_tracker/). This component listens
**passively** to BLE advertisement packets and does not pair with the device, so ESPHome has no impact on battery life.
...
The S200 reports weight only. The S400 also measures impedance and heart rate, and each of its measurement
cycles produces **two BLE packets**:
...
<Image
  src="/images/xiaomi_body_scale.jpg"
  width={300}
  height={298}
  layout="constrained"
  alt=""
...
- **impedance_low** (_Optional_, S400 only): Low-frequency impedance at **50 kHz** in Ω. Numerically larger value
  (~558 Ω). Published together with weight and heart rate.
  - All options from [Sensor](/components/sensor/).
- **impedance_high** (_Optional_, S400 only): High-frequency impedance at **250 kHz** in Ω. Numerically smaller value
  (~503 Ω). Published in the second packet without weight or heart rate.
  - All options from [Sensor](/components/sensor/).
- **heart_rate** (_Optional_, S400 only): Heart rate in bpm.
  - All options from [Sensor](/components/sensor/).
- **profile_id** (_Optional_): User profile slot (1–5, matches the Xiaomi Home app user assignment).
  - All options from [Sensor](/components/sensor/).
- **stabilized** (_Optional_, S400 only): Binary sensor that turns `ON` when the measurement
...
The S200 sends object ID `0x4E16` with the same layout, except that `data[1-4]` holds only the weight × 100
(0.01 kg).
...
The S400 provides raw biometric data (weight + dual-frequency impedance). To compute body composition scores (BMI, body
