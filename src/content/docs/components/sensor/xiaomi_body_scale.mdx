...
Composition Scale S400 using a BLE tracker hub such as the [ESP32 BLE Tracker](/components/esp32_ble_tracker/). This
...
The S200 reports weight and profile ID only. The S400 also measures impedance and heart rate. A bare feet S400
measurement produces **two BLE packets**; with socks, only the first packet is sent:
...
3. Find your scale in the generated list.
   ...
   > **`xiaomi.scales.ms110`**. This mapping is for the S400 only.
