...
- **store_baseline** (*Optional*, boolean): Periodically save the VOC algorithm's learned state to flash memory and restore it after a reboot. The saved state is automatically discarded if the device configuration has changed. Only has effect when a voc sensor is configured.
  Defaults to ``true``.

- **startup_delay** (*Optional*, [Time](/guides/configuration-types#time)): The time to wait after the sensor starts measuring before publishing values, to allow the sensor to stabilise.
  Defaults to `60s`.
