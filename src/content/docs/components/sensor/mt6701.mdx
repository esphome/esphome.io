...
([datasheet](https://www.magntek.com.cn/upload/pdf/202407/MT6701_Rev.1.8.pdf)) 14-bit absolute magnetic angle
...
If five reads in a row fail (a bus error, or over SSI a CRC mismatch), the hub
raises a warning status. It clears on the next good read.
...
- **out_pin** (*Optional*): Analog / PWM output pin settings.
  ...
  - **analog_start** / **analog_stop** (*Optional*): Start/stop angles of the
    analog or PWM output range (12-bit, same units as **zero_offset**).
...
> [`mt6701_i2c.save_eeprom`](#mt6701_i2c-save_eeprom-action) action.

> [!NOTE]
> The I²C pins double as the ABZ outputs, and on the SOP-8 package also as the
> UVW outputs; the chip's `MODE` pin selects which of the two they carry. To use
> ABZ or UVW output that you configure here, save it to EEPROM first and then
> switch the `MODE` pin. The analog / PWM output has a pin of its own.
...
- **update_interval** (*Optional*, [Time](/guides/configuration-types/#time)): The
  interval at which the hub reads the encoder. Defaults to `60s`. The status
  entities below are refreshed on every read, including the reads triggered by
  the [sensor](#sensor).
...
> [!NOTE]
> A disconnected or stuck-low data line reads as all-zero frames, which pass the
> CRC. At boot the hub therefore needs at least one frame with a non-zero angle,
> so an encoder that sits at exactly `0` during setup is reported as not found.
...
survive a power cycle. Only available on the `mt6701_i2c` hub. The hub stops
reading the encoder while the chip programs its EEPROM.

The chip loads its EEPROM at power-up, so the datasheet verifies the
programming by power-cycling the chip and reading the registers back. To check
it, remove the options from your configuration (so they are not written again
at boot), power-cycle the device and confirm the outputs behave as programmed.
...
> also requires the supply voltage to be **4.5-5.5 V** during the programming
> window of more than 600 ms, so a 3.3 V-powered sensor should not be programmed.
