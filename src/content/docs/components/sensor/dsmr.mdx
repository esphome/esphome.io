...
uart:
  ...
  baud_rate: 115200
  # It is recommended to set the rx_buffer_size to at least 1500 bytes to avoid packet losses
  rx_buffer_size: 1700
...
## Troubleshooting. Log the raw input data and enable very verbose logging.

If you have problems with receiving data add these configuration options to `logger` and `uart` components:
```yaml
logger:
  level: VERY_VERBOSE
