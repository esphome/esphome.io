...
And also the following models from [Luftmy](https://luftmy.com/article_list_30.html)
...
[LD16](https://luftmy.com/article_read_88.html)
...
If you use an update_interval > 30 sec, you need to connect the tx_pin to the sensor.
...
| Type         | `PMS1003` | `PMS3003` | `PMSX003`                                  | `PMS5003S` | `PMS5003T` | `PMS5003ST` | `PMS9003M` | `LD1X`          |
| ------------ | --------- | --------- | ------------------------------------------ | ---------- | ---------- | ----------- | ---------- | --------------- |
...
The LD1X from luftmy offer are longer lifetime compared to the Plantower models.
...
```yaml
# Example configuration entry (Luftmy LD1X - LD15 / LD16)
sensor:
  - platform: pmsx003
    type: LD15
    pm_1_0_std:
...
- **pm_1_0** (*Optional*): Use the concentration of particulates of size less than 1.0µm in µg per cubic meter under atmospheric environment.
  Not supported by `LD1X` (`LD15`, `LD16`) type sensors.
  All options from [Sensor](/components/sensor).
