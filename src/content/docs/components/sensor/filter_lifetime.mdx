...
- **is_on** (*Optional*, [lambda](/automations/templates#config-lambda)): A lambda that returns `true` when the device is running. Runtime only accumulates when this returns `true`. Defaults to `true` (always on).
...
- **current_speed** (*Optional*, [lambda](/automations/templates#config-lambda)): A lambda that returns the current operating speed as a percentage (0-100). This scales the runtime accumulation - running at 50% speed accumulates half as much runtime. Defaults to `100.0` (full speed).
...
### `filter_lifetime.reset_filter` Action
            ...
            then:
              - homeassistant.action:
                  ...
                  data:
                    message: !lambda |-
                      char buffer[64];
                      sprintf(buffer, "Filter needs replacement! Only %.0f%% remaining", x);
...
- <APIRef text="filter_lifetime.h" path="filter_lifetime/filter_lifetime.h" />
