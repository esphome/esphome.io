...
- **thresholds** (*Optional*): Override the voltage, current, and frequency thresholds used for ATM90E32 power quality monitoring and text sensor status reporting. If omitted, the defaults preserve the previous behavior: voltage sag and peak remain at `78%` and `122%` of nominal, current peak remains `65.53A`, and frequency limits remain `line_frequency ± 3Hz`.
  ...
  - **voltage_nominal** (*Optional*): Nominal RMS voltage used to derive percentage-based voltage thresholds. Defaults to `120V` for `60Hz` systems and `220V` for `50Hz` systems.
  ...
  - **current_peak** (*Optional*): Over-current threshold in amperes RMS. Defaults to `65.53A`.
  - **frequency_nominal** (*Optional*): Nominal frequency used to derive the frequency thresholds. Defaults to `line_frequency`.
  - **frequency_low_hz** (*Optional*): Absolute low-frequency threshold. Defaults to `frequency_nominal - 3Hz`.
  - **frequency_high_hz** (*Optional*): Absolute high-frequency threshold. Defaults to `frequency_nominal + 3Hz`.

  The effective voltage thresholds must satisfy `sag < nominal < peak`. Frequency thresholds must satisfy `low < high`, and if `frequency_nominal` is set then `low < nominal < high`.

## Thresholds

Use `thresholds` to tune the values written to the ATM90E32 power quality registers and the thresholds used by the status text sensors. Percentage-based voltage thresholds use `voltage_nominal`, while frequency thresholds default to a `±3Hz` band around `line_frequency`.
...
- **phase_status** (*Optional*): Enables per-phase status conditions:
  ...
  - **phase_a** (*Optional*): Phase A status field.
    ...
    - **Over Current**: Triggered when RMS current reaches `thresholds.current_peak`. Defaults to `65.53A`.
...
This setting only affects the `peak_current` sensor output; over-current detection is configured separately with `thresholds.current_peak`.
