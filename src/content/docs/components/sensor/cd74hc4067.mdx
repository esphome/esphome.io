...
Then you need to set up a voltage sensor source (e.g. [ADC sensor](/adc/)) and pass it to the cd74hc4067 sensors with the `sensor` option.
...
- **sensor** (**Required**, [ID](/guides/configuration-types#id)): The source sensor to measure voltage values from, e.g. [ADC sensor](/adc/).
...
For this purpose, [CT clamp sensors](/ct_clamp/) are attached on each of the sensors.
