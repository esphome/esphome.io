...
Standard Check LP Bluetooth Low Energy devices using 
a BLE tracker (for example the [ESP32 BLE Tracker](/components/esp32_ble_tracker/)). This component will track the tank level,
...
ESPHome can identify them. So first, create a simple configuration with a BLE tracker (such as `esp32_ble_tracker`)
