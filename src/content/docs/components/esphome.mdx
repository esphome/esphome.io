...
- **publish_shell_command** (*Optional*, string): A shell command to run when `PUBLISH` is used as the
  upload device, for example with `esphome run <config> --device PUBLISH`. The command runs with the
  config file's directory as the working directory. A number of environment variables are set automatically.
