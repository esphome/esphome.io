...
- **publish_shell_command** (*Optional*, string): A shell command to run on the server when the user
  clicks **Publish** in the ESPHome dashboard install dialog. The **Publish** option only appears in the install
  dialog when this key is set. The command runs with the config file's directory as the working directory. The
  following environment variables are set automatically:

  - ``ESPHOME_DEVICE_NAME`` — the device name from the configuration
  - ``ESPHOME_FIRMWARE_BIN`` — absolute path to the compiled firmware binary
  - ``ESPHOME_MQTT_BROKER``, ``ESPHOME_MQTT_PORT``, ``ESPHOME_MQTT_USERNAME``, ``ESPHOME_MQTT_PASSWORD``,
    ``ESPHOME_MQTT_TOPIC_PREFIX`` — MQTT connection details (only set when an ``mqtt:`` section is present in the config)

  Publishing can also be triggered from the command line with ``esphome upload <config> --device PUBLISH``.

  See the [HTTP OTA via MQTT cookbook](/cookbook/http-ota-mqtt/) for an example of how this can be used.
