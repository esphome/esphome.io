...
- **provisioning** (*Optional*): Configure a provisioning window for devices that
  ship unprovisioned. See [`provisioning`](#esphome-provisioning).
...
<span id="esphome-provisioning"></span>

## `provisioning`

Sets up a **provisioning window** for devices that ship without their credentials
configured — for example a boxed product whose API encryption key is set by Home
Assistant the first time it connects. This helps meet the secure-communication
requirements of standards such as EN18031.

The device opens the provisioning window when it boots. Once it has been
provisioned — the API encryption key has been set — the window closes on its own.
If `timeout` elapses while the device is still unprovisioned, the window closes
anyway and the device stops accepting provisioning:

- New API clients are turned away with a "provisioning closed" reason.
- BLE Improv ([`esp32_improv`](/components/esp32_improv/)) stops accepting Wi-Fi
...
# A device that boots unprovisioned: encryption enabled, no key set.
api:
  encryption:
```
