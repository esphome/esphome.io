...
- **publish_shell_command** (*Optional*, string): A shell command to run on the server when the user
  ...
  dialog when this key is set. The command runs with the config file's directory as the working directory. A
  number of environment variables are set automatically.
  ...
  - ``ESPHOME_MQTT_BROKER``, ``ESPHOME_MQTT_PORT``, ``ESPHOME_MQTT_USERNAME``, ``ESPHOME_MQTT_PASSWORD``, ``ESPHOME_MQTT_TOPIC_PREFIX`` — MQTT connection details (only set when an ``mqtt:`` section is present in the config)
