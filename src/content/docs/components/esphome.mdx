...
`includes_c` behaves the same way, but wraps generated `#include` statements in `extern "C" {}`.
...
esphome:
  ...
  includes_c:
    - path: my_c_header.h
      stage: after_all_globals
```

Each item may be either a plain string or a mapping with these keys:

- **path** (**Required**, string): Relative path to a file or directory, or a system include written with `<...>`.
- **stage** (*Optional*, string): Controls where generated include statements are emitted relative to generated globals.
  Supported values are `before_any_globals` and `after_all_globals`.

If `stage` is omitted, ESPHome keeps the previous behavior:

- System includes such as `<vector>` default to `before_any_globals`.
- Local headers and directories default to `after_all_globals`.

Use `before_any_globals` when a header defines types or macros used by generated globals.
Use `after_all_globals` when a header needs generated globals to exist first.
