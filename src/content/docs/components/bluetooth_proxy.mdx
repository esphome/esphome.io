...
- **LN882x and RP2 (Raspberry Pi Pico W family)** — **advertisement-only**. These controllers do
  not expose the GATT client the proxy needs for connections, so `active` must stay `false` and
  Home Assistant treats the device as a passive scanner. Advertisement forwarding — which is what
  integrations such as BTHome, Xiaomi and most sensors rely on — works exactly as it does on
  ESP32.
...
```yaml
# Advertisement-only proxy on an LN882x or RP2 BLE platform
bluetooth_proxy:
```
