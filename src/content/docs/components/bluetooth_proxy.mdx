...
The proxy runs on any platform that provides a BLE hub, but the feature set depends on what the
controller can do:
...
- **RP2 (Raspberry Pi Pico W family)** — **advertisement-only**. The controller does not expose the
  GATT client the proxy needs for connections, so `active` must stay `false` and Home Assistant
  treats the device as a passive scanner. Advertisement forwarding — which is what integrations
  such as BTHome, Xiaomi and most sensors rely on — works exactly as it does on ESP32.
...
```yaml
# Advertisement-only proxy on an RP2 BLE platform
bluetooth_proxy:
```

> [!NOTE]
> On advertisement-only platforms a scanner-mode request from Home Assistant is applied when the
> tracker supports the requested mode (a running scan restarts with the new mode); otherwise the
> device declines it, logs a single warning and reports its real state back.
