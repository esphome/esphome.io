...
>   - 24-bit / RGB
>
> - JPEG images, baseline and progressive. Progressive JPEG decoding requires the `LIBJPEG_TURBO`
>   decoder (ESP32 only); see [JPEG Decoder](#jpeg-decoder).
>
> - PNG images
...
The JPEG decoder is selected for the whole build with the `jpeg_decoder` option of the `runtime_image` component; it
applies to every JPEG image in the configuration.

```yaml
runtime_image:
  jpeg_decoder: LIBJPEG_TURBO
```

### Configuration Variables

- **jpeg_decoder** (*Optional*): The JPEG decoder to use. Defaults to `JPEGDEC`.

  - `JPEGDEC`  : Supports baseline JPEG images only.
  - `LIBJPEG_TURBO`  : Supports baseline and progressive JPEG images. ESP32 only.

> [!NOTE]
> `LIBJPEG_TURBO` uses roughly 120 kB more flash than `JPEGDEC`. Progressive images require the coefficients of the
> whole image to be buffered in memory, so PSRAM is generally required for large images.
