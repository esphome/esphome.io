...
>   decoder (ESP32 only); see the `decoder` option below.
>
> - PNG images
...
- **decoder** (*Optional*): Selects the JPEG decoder to use. Only valid when `format` is `JPEG`. Defaults to `JPEGDEC`.

  - `JPEGDEC`  : The default decoder. Supports baseline JPEG images only.
  - `LIBJPEG_TURBO`  : Supports baseline and progressive JPEG images. ESP32 only. Replaces the JPEG decoder for the
    whole build, so it applies to every JPEG image in the configuration once selected. Progressive images require
    the coefficients of the whole image to be buffered in memory, so PSRAM is generally required for large images.
