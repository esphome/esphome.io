...
Animations support `type: AUTO` and `target_display` with the same
[selection rules as file images](/components/image/file/#automatic-image-encoding). Every frame is encoded on the
build computer as RGB565 or RGB for the selected target. Requested alpha is retained in either format.
...
When an LVGL image widget displays the animation, refresh its source with `lvgl.image.update` after changing frames.
