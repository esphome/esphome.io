...
The `esp32_ble` component in ESPHome sets up the Bluetooth LE stack on the device so that a [Esp32 Ble Server](/esp32_ble_server/)
can run.
...
- **advertising** (*Optional*, boolean): Manually enable BLE advertising support. This is automatically enabled when using [Esp32 Ble Server](/esp32_ble_server/) or [Esp32 Ble Beacon](/esp32_ble_beacon/). Only set this to `true` if you need advertising functionality without those components. Defaults to `false`.
...
- **max_connections** (*Optional*, integer): The maximum number of simultaneous BLE connections (client + server combined). Defaults to `3`.
  ...
  - This limit is shared between [Esp32 Ble Tracker](/esp32_ble_tracker/) (BLE client), [Bluetooth Proxy](/bluetooth_proxy/), and [Esp32 Ble Server](/esp32_ble_server/) (BLE server)
...
- [Esp32 Ble Server](/esp32_ble_server/)
