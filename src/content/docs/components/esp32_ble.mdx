...
The `esp32_ble` component in ESPHome sets up the Bluetooth LE stack on the device so that a [esp32_ble_server/](/esp32_ble_server/)
can run.
...
- **advertising** (*Optional*, boolean): Manually enable BLE advertising support. This is automatically enabled when using [esp32_ble_server/](/esp32_ble_server/) or [esp32_ble_beacon/](/esp32_ble_beacon/). Only set this to `true` if you need advertising functionality without those components. Defaults to `false`.
...
- **max_connections** (*Optional*, integer): The maximum number of simultaneous BLE connections (client + server combined). Defaults to `3`.
  ...
  - This limit is shared between [esp32_ble_tracker/](/esp32_ble_tracker/) (BLE client), [bluetooth_proxy/](/bluetooth_proxy/), and [esp32_ble_server/](/esp32_ble_server/) (BLE server)
...
<a id="ble-enabled_condition" aria-hidden="true"></a>
