...
- **filter** (*Optional*): Advanced parameters for configuring the filtering behavior.

    - **mac_address** (*Optional*, List) - list of MAC addresses with which BLE should work

> [!NOTE]
> A `mac_address` list is used to restrict which devices can establish a BLE connection with the ESP32 board. In addition, the same list is applied at the scanning stage, so the board only processes advertisement packets from approved devices. This ensures that both connection attempts and incoming BLE advertisements are limited to explicitly authorized devices, improving security and reducing unnecessary processing.
