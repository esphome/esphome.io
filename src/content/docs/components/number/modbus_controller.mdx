...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **payload** (`RegisterValues &payload`): *Deprecated, removed in 2027.3.0 - use `item` instead.*
    ...
    device address. This is the number platform's historical buffer meaning; it differs from select and output,
    whose buffers take plain register words.
...
number:
  ...
  - platform: modbus_controller
    modbus_controller_id: modbus1
    id: battery_capacity_number_lambda
    name: "Battery Cap Number"
    address: 0x9001
    value_type: U_WORD
    lambda: "return  x * 1.0; "
