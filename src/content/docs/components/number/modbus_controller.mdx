...
- **lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **item** (`ModbusNumber *`): The number entity, a [`SensorItem`](https://api-docs.esphome.io/classesphome_1_1modbus__controller_1_1_sensor_item) subclass, so the configured address, `offset`, `bitmask`, etc. are accessible.
...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **item** (`ModbusNumber *`): The entity as its own modbus device: a [`ModbusClientDevice`](https://api-docs.esphome.io/classesphome_1_1modbus_1_1_modbus_client_device) subclass, so every client request helper is available - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the number's own register type. Also a [`SensorItem`](https://api-docs.esphome.io/classesphome_1_1modbus__controller_1_1_sensor_item) subclass, so the configured address, `offset`, `bitmask`, etc. are accessible.
  - **payload** (`RegisterValues &payload`): *Deprecated, removed in 2027.3.0 - use `item` instead.*
    An empty buffer; if the lambda fills it (`push_back`), it is sent as a raw modbus frame packed from the 16 bit words, and must include all required bytes, including the device address.

  Possible return values for the lambda:

  - `return <FLOATING_POINT_NUMBER>;` the value to be written to the device.
    The number still reports the requested value `x`; use this to translate between the ESPHome value and the modbus transmitted value.
...
number:
  ...
  - platform: modbus_controller
    ...
    write_lambda: |-
      ...
      // drive the write directly through the entity; the number still reports x
      item->write_single_register(0x9001, (uint16_t) x);
      return {};
```
