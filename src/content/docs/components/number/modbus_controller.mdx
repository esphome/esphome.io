...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **custom_pdu** (`std::vector<uint16_t> &custom_pdu`): empty vector for a custom PDU. If it is filled in the lambda,
    its contents are sent as a raw PDU (function code + data) instead of the standard write command. The PDU must not
    include the device address; the controller's `address` is used and the CRC is appended automatically.
