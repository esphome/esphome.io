...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **item** (`ModbusNumber *`): The entity as its own modbus device: the write helpers and `queue_pdu()` are available - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the number's own register type. Also a [`SensorItem`](https://api-docs.esphome.io/classesphome_1_1modbus__controller_1_1_sensor_item) subclass, so the configured address, `offset`, `bitmask`, etc. are accessible.
  - **payload** (`RegisterValues &payload`): *Deprecated, removed in 2027.3.0 - use `item` instead.*
    An empty buffer; if the lambda fills it (`push_back`/`clear`/`assign` - not the full `std::vector` API), it is sent as a raw modbus frame packed from the 16 bit words, and must include all required bytes, including the device address.
