...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **item** (pointer to the number object): The number itself.
    The number is its own modbus device, so the lambda can send any write directly through it - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the number's own register type.
  - **payload** (`ModbusWriteRegisters &payload`): *Deprecated, removed in 2027.3.0 - use `item` instead.*
  ...
  - `return <FLOATING_POINT_NUMBER>;` the value written to the device.
    The number still reports the requested value `x`; use this to translate between the ESPHome value and the wire value.
