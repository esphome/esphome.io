...
- **tx_pin** (*Optional*, [Pin](/guides/configuration-types#pin)): The pin to send data to from the ESP's perspective. Use the full pin
  ...
  only the `esp32`/`esp32h2`/`esp32c6` variants support this (see [`zephyr` notes](#zephyr) below); `native_sim` does not.
...
- **port** accepts `uart0` or `uart1`. Defaults to `uart0` if not set.
- **tx_pin**/**rx_pin** may be set together with **port** on the `esp32`/`esp32h2`/`esp32c6` variants, whose UART
  ...
  `native_sim` variant does not support pin selection at all — omit **tx_pin**/**rx_pin** there.
- Ports other than `uart0` have no pinctrl configured on stock development boards, so **tx_pin**/**rx_pin** are
  required together with `port: uart1` on those boards.
