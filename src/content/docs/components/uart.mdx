...
> Unsupported selections are rejected during configuration validation.
> The startup log shows the clock name resolved by ESP-IDF; `DEFAULT` may therefore appear as `APB` or `XTAL`.
