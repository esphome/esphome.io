...
- **clock_source** (*Optional*): ESP32 only. The UART clock source. Options: `DEFAULT`, `APB`, `XTAL`, `RTC`,
  `REF_TICK`. Defaults to `DEFAULT`, which uses the chip's ESP-IDF default. Supported values vary by chip and an
  unsupported selection is rejected during configuration validation.

> [!NOTE]
> The default clock is the right choice unless you have enabled ESP-IDF dynamic frequency scaling yourself, for
> example with `CONFIG_PM_ENABLE` under `sdkconfig_options` in the [esp32](/components/esp32/) component. ESPHome does
> not enable it. With scaling active the APB frequency changes at runtime, so a UART clocked from APB loses its baud
> rate. On those builds pick a source that does not follow APB:
>
> - `XTAL` is the external crystal and keeps the full baud rate range. It is available on every variant except the
>   original ESP32 and ESP32-S2.
> - `REF_TICK` is a fixed 1 MHz clock and the only choice independent of APB on the original ESP32 and ESP32-S2.
>   Because it is so slow the divider error grows above a few hundred kilobaud, so it suits lower baud rates only.
> - `RTC` is the internal RC oscillator, which is uncalibrated and drifts with temperature and supply voltage. It is
>   meant for light sleep use and is not accurate enough for a general serial link.
>
> `DEFAULT` uses APB on the original ESP32, ESP32-S2, ESP32-S3 and ESP32-C3, and a fixed PLL clock on the newer
> variants. The startup log shows the clock ESP-IDF resolved, so `DEFAULT` may appear there as `APB`.
