...
advanced options for the full bundle. Servers using self-signed certificates or private CAs will fail to connect with the built-in bundle. To trust a private CA without disabling verification, point ca_certificate_path at the CA certificate file:
...
The PEM file is baked into the firmware, so re-flash (e.g. OTA update) after rotating the
CA certificate. When `http_request` is configured with its own `ca_certificate_path`, media
sources inherit it automatically, so a single declaration covers both components:
...
media_source:
  - platform: audio_http   # inherits the CA above
```

`verify_ssl: false` under `http_request` remains available for trusted networks, but it
disables certificate verification globally for all TLS connections in the build; preferring
`ca_certificate_path` keeps verification on with your own trust anchor.
...
- **persistent_ring_buffer** (*Optional*, boolean): If `true`, pre-allocate the decoder ring buffer
  ...
  Defaults to `false`.
- **ca_certificate_path** (*Optional*, path): Path to a PEM file containing the CA
  certificate(s) used to verify the server for `https://` playback URLs. The file is read at
  build time and embedded in the firmware, and it replaces — not extends — ESP-IDF's bundled
  certificate store for this media source. Relative paths are resolved against the
  configuration directory. If omitted, the value from the
  [`http_request`](/components/http_request/) component's `ca_certificate_path` is used when
  that component is configured.
