...
- **wakeup_pin** (*Optional*, [Pin Schema](/guides/configuration-types#pin-schema) / list): Only on ESP32/BK72xx/nRF52. A single pin to wake up to once
...
- **wakeup_pin_mode** (*Optional*): Only on ESP32/BK72xx/nRF52. Specify how to handle waking up from a `wakeup_pin` if
...
> [!NOTE]
> These `wakeup_pin_mode` values also apply on nRF52, where they govern entry into System OFF rather than RTC-pin
> wakeup. See the [nRF52](#nrf52) section for the details and restrictions specific to that platform.
...
A `wakeup_pin` may also be configured to wake the device from System OFF, the path used when no `sleep_duration` is
set and Zigbee is not in use, via the GPIO SENSE latch. All three `wakeup_pin_mode` values are supported: `IGNORE`
(default), `KEEP_AWAKE` (defer entering System OFF while the pin is in its active state), and `INVERT_WAKEUP` (flip
the wake level when the pin is already active at sleep time). The pin's `inverted:` flag selects the active level.

> [!IMPORTANT]
> On nRF52 the `wakeup_pin` can only wake the device when `sleep_duration` is omitted (and Zigbee is not in use), so
> that the device enters System OFF. When `sleep_duration` is set, the device instead takes a timed-sleep path that is
> a kernel delay which the GPIO SENSE latch cannot interrupt — the wakeup pin never wakes the device (and `KEEP_AWAKE`
> likewise has no effect). Configuring `sleep_duration` and `wakeup_pin` together therefore emits a validation warning
> at compile time. To resolve it, omit `sleep_duration` to wake from the pin, or remove `wakeup_pin` for a pure timer
> wakeup.

```yaml
# Example pin-only wakeup on nRF52 (no sleep_duration)
deep_sleep:
  run_duration: 15s
  wakeup_pin:
    number: P0.05
