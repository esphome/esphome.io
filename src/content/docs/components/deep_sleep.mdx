...
- **ota_timeout** (*Optional*, [Time](/guides/configuration-types#time)): The time duration to pause deep sleep during an OTA update in case of failure.  Highly recommended to set longer then 2 minutes.
  On successful OTA update device will reboot resuming normal deep sleep behavior.
