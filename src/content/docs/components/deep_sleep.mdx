...
set, via the GPIO SENSE latch. All three `wakeup_pin_mode` values are supported: `IGNORE` (default), `KEEP_AWAKE`
(defer entering System OFF while the pin is in its active state), and `INVERT_WAKEUP` (flip the wake level when the
pin is already active at sleep time). The pin's `inverted:` flag selects the active level.
