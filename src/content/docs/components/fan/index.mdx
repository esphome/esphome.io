...
- **id** (**Required**, [ID](/guides/configuration-types#id)): The ID of the fan.
- **off_speed_cycle** (*Optional*, boolean): Determines if the fan will cycle off after cycling though its highest speed. Can be `true` or `false`. If `false` fan will cycle to its lowest speed instead of turning off. Defaults to `true`.
