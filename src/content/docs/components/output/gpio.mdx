...
- **pin** (**Required**, [Pin Schema](/guides/configuration-types#pin-schema)): The pin to turn on and off. When
  enabling `hold_during_sleep` the driving component (e.g. light) or automation needs to restore the state after
  wake-up. When using a power supply, make sure to enable `hold_during_sleep` there as well.
