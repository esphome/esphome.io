...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **payload**: an empty vector to fill; if filled, the return value is ignored and the payload is sent as a custom command that must include all required bytes for a modbus request. Its element type depends on `register_type`:

    - for `register_type: holding`  : `std::vector<uint16_t> &payload` - add 16-bit raw modbus register words.
    - for `register_type: coil`  : `std::vector<uint8_t> &payload` - add the raw PDU bytes.
