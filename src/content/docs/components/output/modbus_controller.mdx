...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **item** (`ModbusFloatOutput *` / `ModbusBinaryOutput *`): The entity as its own modbus device: the write helpers and `queue_pdu()` are available - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the output's own register type. Also a [`SensorItem`](https://api-docs.esphome.io/classesphome_1_1modbus__controller_1_1_sensor_item) subclass, so the configured address, `offset`, `bitmask`, etc. are accessible.
  - **payload**: *Deprecated, removed in 2027.3.0 - use `item` instead.*
    An empty buffer to fill (`push_back`/`clear`/`assign` - not the full `std::vector` API); if filled, the return value is ignored and the payload is sent instead. Its element type, and what the bytes mean, depend on `register_type`:
