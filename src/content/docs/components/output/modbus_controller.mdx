...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  Lambda is evaluated before the modbus write command is created. If you fill the supplied vector, the return value is
  ignored and the vector contents are used instead.
  ...
  - For `register_type: holding` — **payload** (`std::vector<uint16_t> &payload`): empty vector for the register
    values. The lambda can add 16 bit raw modbus register words which are written to the output's `address`.
  - For `register_type: coil` — **custom_pdu** (`std::vector<uint8_t> &custom_pdu`): empty vector for a custom PDU.
    If it is filled in the lambda, its contents are sent as a raw PDU (function code + data). The PDU must not include
    the device address; the controller's `address` is used and the CRC is appended automatically.
  ...
  - `return <anything>;` and fill the supplied vector with data — for `holding` these 16 bit register words are written,
    for `coil` the PDU is sent.
