...
- **nvs_cache_in_psram** (*Optional*, boolean): Keep the NVS page cache and key hash lists in PSRAM instead of
  internal RAM; preference values stay in flash. Frees about 10 KB of internal heap on ESPHome's default NVS
  partition. NVS operations get roughly 2.5 times slower, which a handful of preference reads and writes do not
  notice; see
  [Espressif's documentation](https://docs.espressif.com/projects/esp-idf/en/v5.5.5/esp32/api-reference/storage/nvs_flash.html).
  Enabled by default when [PSRAM](/components/psram/) is configured with `ignore_not_found: false` (its default is
  `true`, which leaves this off) and [`nvs_encryption`](#nvs-encryption) is off; with encryption on the cache stays
  internal so key material never lands in PSRAM. Set to `false` to keep the cache internal; an explicit `true` where
...
- **signed_ota_verification** (*Optional*, mapping): Enable
  ...
  > [!NOTE]
  > A device built this way also advertises `ota_signed=1` in its [mDNS](/components/mdns/) TXT records, meaning
  > it accepts only updates signed by a key it trusts. Tools that discover devices on the network can use this
  > to check up front whether they can sign an update at all, and say that a serial flash is needed when they
  > cannot. A missing record does not mean the device accepts unsigned updates: it may have mDNS disabled or run
  > firmware from before the record was added.
