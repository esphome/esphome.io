...
- **partitions** (*Optional*, filename or list): A partition table CSV file or a list of custom partitions to add.
  See [Partitions](#esp32-partitions) below.
...
provide a custom partition table CSV file, or add extra partitions to the auto-generated table.

When using a list of partitions, custom partitions are appended to the auto-generated table. The app partition size is automatically
reduced to make room. Valid partition types and subtypes are documented in the
...
Alternatively, you can provide a path to a CSV file for full control over the partition layout:
