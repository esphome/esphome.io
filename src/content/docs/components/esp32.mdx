...
- **signed_ota_verification** (*Optional*, mapping): Enable
  ...
  > [!NOTE]
  > A device built this way also advertises `ota_signed=1` in its [mDNS](/components/mdns) TXT records. Tools
  > that discover devices on the network can use this to warn, before an update is attempted, that firmware
  > they compile themselves has to be flashed over serial — they have no way to sign it.
