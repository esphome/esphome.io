...
- **item** (`ModbusSelect*`  ): The select object itself.
  The select is its own modbus device, so the lambda can send any write directly through it - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the select's own register type.
