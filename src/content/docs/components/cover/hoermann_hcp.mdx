...
The `hoermann_hcp` text sensor platform reports the motor's serial number and firmware version. The
motor is asked once after the device starts, and the values appear a moment later. A value the motor
does not send after three attempts, 30 seconds apart, stays unknown and a warning is logged.
...
- **serial_number** (*Optional*): The motor's serial number. Defaults to the `diagnostic` entity
  category.

- **version** (*Optional*): The motor's firmware version. Defaults to the `diagnostic` entity
  category.
