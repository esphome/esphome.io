...
- **ca_certificate_path** (*Optional*, file path): Path to a PEM-encoded CA certificate file. Use this to verify
  ...
  included, reducing firmware size. *Also inherited by [audio_http](/components/media_source/audio_http/) media sources that do not set their own `ca_certificate_path`.*
