...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **x** (bool): The boolean state requested for the switch
  - **item** (pointer to the switch object): The switch itself.
    The switch is its own modbus device, so the lambda can send any write directly through it - for example `item->write_single_register(address, value)` or `item->queue_pdu(...)` - regardless of the switch's own register type.
  - **payload** (`ModbusWriteBytes &payload`): *Deprecated, removed in 2027.3.0 - use `item` instead.*
  ...
  - `return true;` or `return false;` the value written to the device.
    The switch still reports the requested state `x`; return a different value to translate between the ESPHome state and an inverted wire value.
