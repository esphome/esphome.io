...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)): Lambda called before send.
  ...
  - **custom_pdu** (`std::vector<uint8_t> &custom_pdu`): empty vector for a custom PDU. If it is filled in the lambda,
    its contents are sent as a raw PDU (function code + data). The PDU must not include the device address; the
    controller's `address` is used and the CRC is appended automatically.
  ...
  - `return <anything>;` and fill `custom_pdu` with data if a PDU is added from the lambda then it will be sent.
  - `return {};` in the case the lambda handles the sending of the value itself.

- **custom_pdu** (*Optional*, list of bytes): raw PDU (function code + data) for a modbus command. This allows using non-standard commands. If `custom_pdu` is used `address` and `register_type` can't be used.
  The PDU must not include the device address; the controller's `address` is used and the CRC is automatically calculated and appended to the command.
  See [Using `custom_pdu`](/components/modbus_controller#modbus_custom_pdu) how to use `custom_pdu`
...
switch:
  - platform: modbus_controller
    ...
    write_lambda: |-
      ...
      // custom_pdu is a PDU: function code + data, no device address
      custom_pdu.push_back(0x5);  // force single coil (function code)
      custom_pdu.push_back(0x00); // high byte address of the coil
      custom_pdu.push_back(0x6);  // low byte address of the coil
      custom_pdu.push_back(0xFF); // ON = 0xFF00 OFF=0000
      custom_pdu.push_back(0x00);
      return true;
```
