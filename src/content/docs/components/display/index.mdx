...
LVGL supports [RGB888 output](/components/lvgl/#rgb888-output) on display paths that preserve 24-bit color.
The framebuffer depth, panel interface mode and [embedded image format](/components/image/file/#automatic-image-encoding)
are separate settings; configure them together when using full-color output.
