...
- **color_depth** (*Optional*): The color depth of the display buffer, expressed in bits. Options are `16` (default)
  and `24`. Preferably should be the same as the `pixel_mode` option. Native
  [RGB888 LVGL output](/components/lvgl/#rgb888-output) requires both `color_depth: 24` and `pixel_mode: 24bit`;
  setting only one to 24 bits does not preserve a full 24-bit path.
