...
- **power_down_peripherals** (*Optional*, boolean): For disabled peripherals, automatically save and restore peripheral states, which allows the peripherals to be powered down.  Defaults to True.  See discussion below
...
When power_down_peripherals: True, The device will automatically save/restore register context during sleep/wakeup to make the upper layer unaware of the peripheral powerdown during sleep.  If not enabled (False), no attempt is made to store the register state.  Either setting requires testing since not all peripherals are supported and unsupported peripherals will basically be reset upon wakeup.  It has been observed on a esp32-c6 devkit that setting to False cause a crash with the first minute of operation.
