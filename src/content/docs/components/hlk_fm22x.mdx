...
- **name** (**Required**, string, [templatable](/automations/templates)): The name associated with the face. Up to 31 printable ASCII characters.
  Other characters are refused: the component strips anything else from the module's replies to remove its padding
  bytes, so such a name would come back shortened.
...
direction enrollment. The interrupted scan or enrollment reports error `2` (aborted) through its failed trigger, and
so does every scan or enrollment that was still waiting in the queue behind it.
...
component sends the next one as soon as the previous one has been answered. Up to six commands can wait at once; a
scan or enrollment that does not fit is dropped and reports error `7` through its failed trigger. If the module stops
answering, the component reports a warning after the first unanswered command and an error after three, and keeps
retrying every few seconds until the module is back.
...
| 5    | Unknown error                                                         |
| 6    | Invalid parameter                                                     |
| 7    | The module is out of memory, or the component's command queue is full |
