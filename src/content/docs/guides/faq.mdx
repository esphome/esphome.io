...
Start with the `reason='...'` value in the `Disconnected ssid='...'` log line: it usually narrows the problem down to
credentials, signal strength or the access point itself. See
[Disconnect Reasons](/components/wifi/#disconnect-reasons) on the WiFi component page.
