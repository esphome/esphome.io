...
_Can be issued multiple times._
...
**`--address ADDRESS`**
: Manually set the address to bind to (defaults to 0.0.0.0). The value can be either an IP address or a network interface name (for example, eth0).
If a network interface is specified, the dashboard will bind to all IPv4 and IPv6 addresses assigned to that interface.
