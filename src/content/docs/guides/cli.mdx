...
: Force a specific OTA platform when uploading firmware. By default, ESPHome uses the OTA platform
configured in the device's YAML; if both `platform: esphome` and `platform: web_server` are configured,
the native `esphome` protocol is preferred. Use `--ota-platform web_server` to upload via the
