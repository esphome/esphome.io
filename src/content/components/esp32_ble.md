...
The `esp32_ble` component in ESPHome sets up the Bluetooth LE stack on the device so that a [Link](esp32_ble_server/)
can run.
...
- **advertising** (*Optional*, boolean): Manually enable BLE advertising support. This is automatically enabled when using [Link](esp32_ble_server/) or [Link](esp32_ble_beacon/). Only set this to `true` if you need advertising functionality without those components. Defaults to `false`.
...
- **max_connections** (*Optional*, integer): The maximum number of simultaneous BLE connections (client + server combined). Defaults to `3`.
  ...
  - This limit is shared between [Link](esp32_ble_tracker/) (BLE client), [Link](bluetooth_proxy/), and [Link](esp32_ble_server/) (BLE server)
...
- [Link](esp32_ble_server/)
- [Link](esp32_improv/)
- ::apiref{text="esp32_ble/ble.h" path="esp32_ble/ble.h"}
