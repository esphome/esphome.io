...
- **microphone** (**Required**, [Microphone Source Configuration](/components/microphone#config-microphone-source)): The
  [microphone](/components/microphone/index) settings to use for input.
...
- **speaker** (*Optional*, [ID](/guides/configuration-types#id)): The [speaker](/components/speaker/index) to use to output
  the response. Cannot be used with `media_player` below.

- **media_player** (*Optional*, [ID](/guides/configuration-types#id)): The [media_player](/components/media_player/index)
...
- **wake_word** (*Optional*, string): The wake word that was used to trigger the voice assistant
  when using on-device wake word such as [micro_wake_word](/components/micro_wake_word).
...
Here is an example offering Push to Talk with a [binary_sensor](/components/binary_sensor).
...
- [Link](microphone/)
- ::apiref{text="voice_assistant/voice_assistant.h" path="voice_assistant/voice_assistant.h"}
