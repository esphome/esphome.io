...
The `mixer` speaker platform allows you to mix audio sent to different source speakers into one output which is sent to another [speaker component](/components/speaker/index). Individual source speakers may be ducked (made quieter) with the [apply ducking action](#mixer_speaker-apply_ducking).
...
- **output_speaker** (**Required**, [ID](/guides/configuration-types#id)): The [speaker](/components/speaker/index) to output the mixed audio.
