...
voltage sensor (like the [ADC sensor](adc)) and convert the readings to measured single phase AC current.

First, you need to set up a voltage sensor source ([ADC sensor](adc), but for example also
[ADS1115](ads1115)) and pass it to the CT clamp sensor with the `sensor` option.
...
- [Link](adc/)
- [Link](ads1115/)
- ::apiref{text="sensor/ct_clamp_sensor.h" path="sensor/ct_clamp_sensor.h"}
