...
To use this sensor, you need to setup a voltage sensor source ([ADC sensor](adc), but for example also [ADS1115](ads1115)) and pass it to the sensor with the `sensor` option, in order to measure the sensor's output voltage pulses; you also need a binary [Output Component](/components/output#output) to control the sensor's internal IR LED.
...
- ::apiref{text="gp2y1010au0f/gp2y1010au0f.h" path="gp2y1010au0f/gp2y1010au0f.h"}
