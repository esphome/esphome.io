...
The `sound_level` sensor platform allows you to measure a [microphone](/components/microphone/index)'s average and peak sound pressure levels over a specified measurement duration. The sensors output in **relative** `dB`, where `0 dB` represents the loudest sound the microphone can measure.
...
- **microphone** (**Required**, [Microphone Source Configuration](/components/microphone#config-microphone-source)): The [microphone](/components/microphone/index) settings to use for input. Multiple channels may be selected.
...
- ::apiref{text="sound_level/sound_level.h" path="sound_level/sound_level.h"}
