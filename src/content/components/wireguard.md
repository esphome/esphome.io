...
>
> See [Link](time/) to setup a time source
...
>
> The [sntp](time/sntp) is a valid time-platform choice.
...
- **address** (**Required**, IPv4 address): The local VPN address of the device.
  ...
  to copy this value to the `use_address` parameter of the [Link](wifi/).
...
- **netmask** (*Optional*, IPv4 address): The netmask for the configured address.
  ...
  expected to transit through the VPN link (e.g. with [Link](mqtt/) to a
  remote MQTT broker).
...
- **require_connection_to_proceed** (*Optional*, boolean): Set to `true` to
  ...
  [Link](mqtt/) to reach a remote broker you may experience boot freeze just
...
> the `use_address` parameter of the [Link](wifi/) to the value
