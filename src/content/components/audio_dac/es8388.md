...
title: ES8388
description: >-
  Instructions for using ESPHome's es8388 audio DAC platform to play media from
  your devices and to configure microphone inputs.
seo:
  description: >-
    Instructions for using ESPHome's es8388 audio DAC platform to play media
    from your devices and to configure microphone inputs.
  image: es8388.svg
---
...
[media_player](/components/media_player).

It also allows your ESPHome devices to use the `es8388`  high performance two channel audio ADC.

This allows attached microphones to be used as a microphone input via [i2s_audio](/components/microphone/i2s_audio).
...
The `es8388` select allows you to control the `dac output` and the `adc input_mic` of your [Link](es8388/).
...
- **es8388_id** (*Optional*, [ID](/guides/configuration-types#id)): Manually specify the ID for the [Link](es8388/) component.
...
- [Link](index/)
- ::apiref{text="es8388/es8388.h" path="es8388/es8388.h"}
- ::apiref{text="audio_dac/audio_dac.h" path="audio_dac/audio_dac.h"}
