...
- **channel** (*Optional*, int): The Wi-Fi channel that the esp-now communication will use to send/receive data packets.
  Cannot be set when the [Link](wifi/) is used, as it will use the same channel as the wifi network.
...
- ::apiref{text="espnow/espnow.h" path="espnow/espnow.h"}
- [espnow](/components/packet_transport/espnow)
