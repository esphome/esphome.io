 ...
 */
function processConfigList(listNode, schema, sourceFile, docTitle, seeAlsoSlug, parentSchema) {
  ...
  for (const item of listNode.children ?? []) {
    ...
    if (head.kind === 'prop') {
      ...
      if (nestedList) {
        ...
        if (subSchema) {
          processConfigList(nestedList, subSchema, sourceFile, docTitle, seeAlsoSlug, schema);
...
function getSubSchema(configVar) {
  ...
  if ((configVar.type === 'schema' || configVar.type === 'trigger') && configVar.schema) {
...
function isOddityNotSpecificComponent(folder, file) {
  ...
  if (folder === 'display') {
    return ['lcd_display', 'ssd1306', 'ssd1322', 'ssd1325', 'ssd1327', 'ssd1331', 'ssd1351', 'st7567'].includes(file);
  }
...
export function processFile(data) {
  ...
  const SKIP_TYPES = new Set(['mdxjsEsm', 'mdxJsxFlowElement', 'heading', 'code', 'blockquote', 'html', 'thematicBreak', 'table']);
  for (const c of topChildren) {
    ...
    if (c.type === 'paragraph') {
      ...
      if (isConfigVarsHeading({ type: 'heading', children: [{ type: 'text', value: text }] })) break;
      if (text) { firstParaDoc = text; break; }
    }
  }
  ...
  if (fileName in core.components) {
    ...
    if (esphomeJson && firstParaDoc) {
      ...
      esphomeJson.core.components[fileName].docs = `${convertLinks(firstParaDoc)}\n\n${seeAlso}`;
      stats.coreDocs++;
    }
    configComponent = fileName;
    isComponent = true;
  } else if (contentFolder !== 'components' && contentFolder in core.platforms) {
    if (fileName === 'index') {
      // Platform index (e.g. sensor/index.mdx) → docs go to esphome.json core.platforms[name]
      const esphomeJson = loadSchema('esphome');
      if (esphomeJson && firstParaDoc) {
        const seeAlso = makeSeeAlso(sourceFile, docTitle, null);
        esphomeJson.core.platforms[contentFolder].docs = `${convertLinks(firstParaDoc)}\n\n${seeAlso}`;
      ...
      if (platformJson && platformJson[contentFolder]?.components?.[fileName] != null) {
        if (firstParaDoc) {
          const seeAlso = makeSeeAlso(sourceFile, docTitle, null);
  ...
  while (i < topChildren.length) {
    ...
    if (node.type === 'heading') {
      ...
      if (isConfigVarsHeading(node)) {
        ...
        if (!configSchema && debugLevel > 1) {
          console.log(`  [warn] ${fileName}: no schema for config vars at "${text}"`);
        }
      ...
      if (acResult) {
        ...
        titleSchema = resolveActionConditionSchema(configType, parts, configComponent, text);
        ...
        if (titleSchema) {
          ...
          console.log(`  [miss] ${fileName}: ${configType} "${text}" not found`);
        }
        i++;
        continue;
      }
      ...
      if (compName) {
        ...
        const result = handleComponentHeading(
          compName, platformName, sourceFile, docTitle, topChildren[i + 1]
        );
      ...
      if (normalized2.endsWith('Over SPI')) {
        const result = handleComponentHeading(
          `${fileName}_spi`, platformName, sourceFile, docTitle, topChildren[i + 1]
        );
        if (result.configComponent) configComponent = result.configComponent;
        i++;
        continue;
      }
      if (normalized2.endsWith('Over I²C') || normalized2.endsWith('Over I2C')) {
        const result = handleComponentHeading(
          `${fileName}_i2c`, platformName, sourceFile, docTitle, topChildren[i + 1]
        );
        if (result.configComponent) configComponent = result.configComponent;
        i++;
        continue;
      ...
      const platform = titleToPlatform(normalized2, configComponent ?? fileName);
      if (platform && fileName !== 'index') {
        platformName = platform;
        const compForPlatform = configComponent ?? fileName;
        const result = handlePlatformComponentHeading(
          compForPlatform, platform, sourceFile, docTitle, topChildren[i + 1]
        );
        if (result.configComponent) {
          configComponent = result.configComponent;
          isComponent = result.isComponent;
    ...
    if (node.type === 'list' && inConfigVars && configSchema) {
      processConfigList(node, configSchema, sourceFile, docTitle, seeAlsoSlug, null);
      i++;
      continue;
    }
...
function resolveConfigSchema(titleSchema, configComponent, isPlatform, isComponent, platformName) {
  ...
  if (isPlatform && configComponent) {
    ...
    if (platformName) {
      return jsonConfig[`${configComponent}.${platformName}`]?.schemas?.CONFIG_SCHEMA?.schema ?? null;
    }
  }
...
function resolveActionConditionSchema(configType, parts, configComponent, titleText) {
  const core = getCore();
  ...
  if (parts.length === 3) {
    ...
    if (!(parts[1] in core.components)) {
      if (debugLevel > 1) console.log(`  [warn] action "${titleText}": "${parts[1]}" not in components`);
    }
...
function handleComponentHeading(compName, platformName, sourceFile, docTitle, nextNode) {
  const core = getCore();
  const desc = nextNode?.type === 'paragraph'
  ...
  if (platformName && platformName in core.platforms) {
    ...
    if (platformJson?.[platformName]?.components?.[compName] != null) {
      if (desc) {
        platformJson[platformName].components[compName].docs = `${desc}\n\n${seeAlso}`;
        stats.platformDocs++;
      }
      return { configComponent: compName, isComponent: false, isPlatform: true };
    }
  }
...
function handlePlatformComponentHeading(compName, platform, sourceFile, docTitle, nextNode) {
  const core = getCore();
  const desc = nextNode?.type === 'paragraph'
    ? convertLinks(paragraphToMarkdown(nextNode))
    : null;
  ...
  if (platform in core.platforms) {
    ...
    if (platformJson?.[platform]?.components?.[compName] != null) {
      if (desc) {
        platformJson[platform].components[compName].docs = `${desc}\n\n${seeAlso}`;
        stats.platformDocs++;
      }
      return { configComponent: compName, isComponent: false, isPlatform: true };
    }
