...
- **ignore_faults** (*Optional*, boolean): If set to ``false``, the sensor will report a ``nan`` value on VCC or ground faults and report a fault warning. If set to ``true``, the faults are ignored (because they are a common occurence with high temperatures and low-cost break-out boards) and the temperature reading is reported as normal. Disconnect faults are never ignored. Defaults to ``false``.
