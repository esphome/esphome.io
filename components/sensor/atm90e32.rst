...
The `CircuitSetup 6-Channel Energy Monitor <https://circuitsetup.us/product/expandable-6-channel-esp32-energy-meter/>`__ can read 6 current channels and 2 voltage channels at a time, this board has two ATM90E32 ICs and requires two sensors to be configured in ESPHome.
...
- **phase_a** (*Optional*): The configuration options for the 1st phase.
  ...
  - **gain_voltage** (*Optional*, int): Voltage gain to scale the low voltage AC transformer to household mains feed.
    Defaults to ``7305``.
...
- **chip_temperature** (*Optional*): Use the chip temperature value. All options from
  :ref:`Sensor <config-sensor>`.
- **reference_voltage** (*Optional*, float): Input for fine grained gain calibration of a known voltage of the voltage reference. 
  It is used to calculate the value for ``gain_voltage``. All options from 
  :ref:`Template Number <template-number>`
...
- **enable_gain_calibration** (*Optional*, boolean): If true this enables gain calibration for voltage, ``gain_voltage``, and current, ``gain_ct``,
  ...
  when more than one is defined. After running, the values are stored in chip memory.
- **enable_offset_calibration** (*Optional*, boolean): If true this enables fine grained offset noise 0 level calibration for voltage, 
  ...
  After running, the values are stored in chip memory. Offset calibration can only be performed when all voltage and current 
  inputs are at a 0 value. USB power is recommended. For power offset calibration, only a voltage transformer should be connected.
...
Calibration Reference Current & Voltage
---------------------------------------
...
- **id** (*Required*, :ref:`config-id`): The ID of the atm90e32 defined above. Required if there are multiple atm90e32 configured.
- **reference_voltage** (*Optional*): Fields for providing a known reference voltage for calibration of the ``gain_voltage`` parameter.
  To be used in conjunction with the ``run_gain_calibration`` button. ``enable_gain_calibration`` must be ``True``
  Fields default to ``min_value: 100``, ``max_value: 200``, ``mode: box``, ``optimistic: True``, ``step: 0.1``.

  - **phase_a** (*Optional*): The phase this field is assigned to for calibration. All options from :ref:`Number <config-number>`.
  - **phase_b** (*Optional*): The phase this field is assigned to for calibration. If not specified, will take use the value from 
    ``phase_a``. All options from :ref:`Number <config-number>`.
  - **phase_c** (*Optional*): The phase this field is assigned to for calibration. If not specified, will take use the value from 
    ``phase_a``. All options from :ref:`Number <config-number>`.

- **reference_current** (*Optional*): Fields for providing a known reference current for calibration of the ``gain_ct`` parameter.
  To be used in conjunction with the ``run_gain_calibration`` button. ``enable_gain_calibration`` must be ``True``
  Fields default to ``min_value: 1``, ``max_value: 200``, ``mode: box``, ``optimistic: True``, ``step: 0.1``.

  - **phase_a** (*Optional*): The phase this field is assigned to for calibration. All options from :ref:`Number <config-number>`.
  - **phase_b** (*Optional*): The phase this field is assigned to for calibration. All options from :ref:`Number <config-number>`.
  - **phase_c** (*Optional*): The phase this field is assigned to for calibration. All options from :ref:`Number <config-number>`.
