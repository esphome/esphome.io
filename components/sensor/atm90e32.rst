...
- **run_gain_calibration** (*Optional*): A button to run the gain calibration. ``reference_current`` and ``reference_voltage``
  should be defined. ``enable_gain_calibration`` must be ``True``. These values can be permanently stored in ``gain_voltage`` 
  and ``gain_ct``.
...
- **id** (*Required*, :ref:`config-id`): The ID of the atm90e32 defined above. Required if there are multiple atm90e32 configured.
- **reference_voltage** (*Optional*):
    - **phase_a** (*Optional*): The phase this field is assigned to. All options from :ref:`Number <config-number>`.
- **reference_current** (*Optional*): 
    - **phase_a** (*Optional*): The phase this field is assigned to
