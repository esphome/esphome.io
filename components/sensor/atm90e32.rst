...
- **gain_pga** (*Optional*, string): Increase this when the output of the CT clamp is too low and ``gain_ct`` is maxed out. One of ``1X``, 
  ``2X``, ``4X``. Defaults to ``1X``, which is suitable for the popular SCT-013-000 clamp.
...
- **phase_status** (*Optional*): Reads out certain conditions detected by the meter: Over Current (>=65.53A), Over Voltage,
  Voltage Sag, and Phase Loss. If none of these conditions, the output will be "Okay".
