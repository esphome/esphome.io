...
- **id** (*Required*, :ref:`config-id`): The ID of the ``atm90e32`` sensor defined above. Required if using more than one sensor.
- **phase_status** (*Optional*): Enables per-phase status conditions:
  ...
  - **phase_a** (*Optional*): Phase A status field. Will report:
    ...
    - **Phase Loss**: Indicates if the phase has dropped out.
      ...
      If no condition is met, the sensor reports ``Okay``. All options from :ref:`Text Sensor <config-text_sensor>`
