...
- **settling_time** (*Optional*, :ref:`config-time`): The settling time after power-up, gain, or channel changes.
  Defaults to ``400ms``. Minimum ``0ms``, maximum ``65535ms``.
- **settle_on_boot** (*Optional*, boolean): Whether to apply the configured settling time on startup.
  Defaults to ``true``. Set to ``false`` if the HX711 is already settled externally or to reduce startup delay.
