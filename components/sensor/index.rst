...
With the ``optimistic`` option enabled, new values are also sent immediately
when they arrive, without waiting for the interval. In addition, you can
restrict which values should be repeated during the interval.  
Note that these conditions do **not** affect immediate forwarding of new values.

Configuration variables:

- **period** (*Required, Time*): Interval at which to repeat values.
- **optimistic** (*Optional, boolean*):  
  - ``false`` (default): Only send last value at the interval.  
  - ``true``: Forward new values immediately and also repeat last value in the interval.
- **value** (*Optional, float*): Only repeat if the last value equals this value.  
- **min_value** (*Optional, float*): Only repeat if the last value is greater or equal.  
- **max_value** (*Optional, float*): Only repeat if the last value is less or equal.  

Note: ``value``, ``min_value`` and ``max_value`` can only be used when ``optimistic: true``.
...
.. code-block:: yaml
    ...
    filters:
      # Repeat last value every 100ms, effectively throttling new values by period
      # New values are only forwarded after the period passed.
      - heartbeat: 100ms
      ...
      # Repeat last value every 100ms. Forward new value immediately.
      - heartbeat:
          period: 100ms
          optimistic: true

      # Repeat only when last value is exactly 0. New values are forwarded immediately even if not 0.
      - heartbeat:
          period: 100ms
          optimistic: true
          value: 0

      # Repeat only when last value is >= 100 and <= 2000.
      # New values are forwarded immediately even if outside this range.
      - heartbeat:
          period: 500ms
          optimistic: true
          min_value: 100
