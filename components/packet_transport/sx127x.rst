.. sx127x-packet-transport:

SX127x Packet Transport Platform
================================

.. seo::
    :description: Instructions for setting up a sx127x packet transport platform on ESPHome
    :image: sx127x.png
    :keywords: sx127x, packet, transport

The :ref:`packet-transport` platform allows ESPHome nodes to directly communicate with each over a communication channel.
The sx127x implementation of the platform uses FSK/OOK or LoRa as a communication medium. See the :ref:`packet-transport`
and :ref:`sx127x` for more information.

Example Configuration
---------------------

.. code-block:: yaml

    # Example configuration entry
    packet_transport:
      platform: sx127x
      sensors:
        - dht_temp

    sx127x:
      dio0_pin: GPIO26
      cs_pin: GPIO18
      rst_pin: GPIO23
      pa_pin: BOOST
      pa_power: 8
      bandwidth: 125_0kHz
      crc_enable: true
      frequency: 433920000
      modulation: LORA
      rx_start: true
      sync_value: 0x33
      spreading_factor: 7
      coding_rate: CR_4_5

    sensor:
      - platform: dht
          id: dht
          pin: GPIOXX
          temperature:
            name: "Temperature"
            id: dht_temp


See Also
--------

- :ref:`packet-transport`
- :doc:`/components/sx127x`
- :doc:`/components/binary_sensor/packet_transport`
- :doc:`/components/sensor/packet_transport`
- :ref:`automation`
- :apiref:`packet_transport/packet_transport.h`
- :ghedit:`Edit`
