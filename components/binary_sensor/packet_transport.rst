...
-  **type** (*Optional*, string): With ``type: status``, the sensor will report the connection status to the referenced provider node (online/offline). Defaults to ``type: data`` where a remote entity value is used.
