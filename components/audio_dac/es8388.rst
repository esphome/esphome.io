...
- **dac_output** (*Required*): Control the DAC Audio output. Defaults to ``LINE1``. It can be set to ``LINE1``, ``LINE2`` or
  ``BOTH``. All options from :ref:`Select <config-select>`.
- **adc_input_mic** (*Required*): Control the ADC Mic Input. It can be set to ``LINE1``, ``LINE2`` or
  ``DIFFERENCE``. All options from :ref:`Select <config-select>`.

Example configuration:
----------------------

Here is the complete example configuration for :doc:`es8388`.
