...
.. note::
    ...
    If you attempt to use this component and experience repeated out-of-memory (OOM) crashes, this is likely the cause of the issue.
    Using framebuffer ``fragmentation`` might help.
...
- **offset_width** (**Required**, int): When ``model`` is set to "Custom", use this to specify the display's horizontal
  offset in pixels. This option may not be specified when the ``model`` is not set to "Custom". Defaults depends ``model``.
- **fragmentation** (**Optional**, int): Reduces RAM usage by breaking updates in smaller fragments, with a reduced framebuffer. Enables low-RAM devices, but has a side-effect that ``lambda`` will be called once per fragment (not per frame). Defaults to ``1``.
