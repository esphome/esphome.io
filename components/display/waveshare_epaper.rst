...
- **model** (**Required**): The model of the E-Paper display. Options are:
  ...
  - ``gdew0213t5d`` - GoodDisplay GDEW0213T5D, as used on the LilyGo T5 Q426
...
- **full_update_every** (*Optional*, int): E-Paper displays have two modes of switching to the next image: A partial
  ...
  because artifacts accumulate. On the ``1.54in``, ``1.54inv2``, ``2.13in``, ``2.13inv2``, ``2.90in``, ``2.90inv2``,  ``7.50inV2p``,  ``gdew029t5`` and ``gdew0213t5d`` models, you have the option to only
