...
.. note::
    ...
    You may have calls to deep_sleep.allow or enter from other places which can re-enable sleep. An on_value handler based on a periodically checked sensor value (battery level okay: prevent, low: allow), or some other automation from HA. In that case this can be overriden and the device goes to deep sleep even so. To be absolutely sure it never happens, you can use this switch. When enabled and the timer reaches run_duration, deep sleep will be postponed until it is disabled again. Any call to deep_sleep.enter will be ignored, too.
