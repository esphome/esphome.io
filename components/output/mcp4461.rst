...
For mcp4461 component itself (not the wiper!):
**************************************

- (uint16_t) get_eeprom_value(uint8_t location):
    get current eeprom user data value of given eeprom-userdata-location (0-4) // 5x 9bits, max int val = 511 ! // ToDO maybe improve eeprom usage
...
**************************************
- (void) write_state(float state): // (protected, override, default frontend function)
    expects value in range 0.00 to 0.256, higher values will be clamped to max value
- (float) read_state(): // returns current wiper state, without (!) updating internal wiper state of output
    returns current hw state of this wiper // NOT the internal state of the output
- (float) update_state():
    returns current value of this output (wiper) & updates internal state of output to this fetched value
- (void) enable_wiper():
