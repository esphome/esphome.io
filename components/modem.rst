...
Because the device will be on a private network, you will need to use :doc:`/components/mqtt` for communication, or the :doc:`/components/api` combined with :doc:`/components/wireguard`.

This component cannot be used at the same time as the :doc:`/components/ethernet` or :doc:`/components/captive_portal` components. It is also not compatible with Wi-Fi station mode.
...
- **rx_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): The pin used for ``RX`` on the ESP side (which should be connected to the ``TX`` pin on the modem).
- **tx_pin** (**Required**, :ref:`Pin Schema <config-pin_schema>`): The pin used for ``TX`` on the ESP side (which should be connected to the ``RX`` pin on the modem).
...
- **power_pin** (*Optional*, :ref:`Pin Schema <config-pin_schema>`): The pin connected to the modem's ``PWK`` or power key, used for power management.
- **status_pin** (*Optional*, :ref:`Pin Schema <config-pin_schema>`): The pin connected to the modem's ``STATUS`` output, used to read the power state.
- **enable_cmux** (*Optional*, boolean): Enables CMUX mode, which allows the modem to answer AT commands while connected to the network. This is required by other components that communicate with the modem, such as :doc:`/components/sensor/modem`. Defaults to ``true``.
- **reboot_timeout** (*Optional*, :ref:`config-time`): The amount of time to wait after a failed connection attempt before rebooting the node. Defaults to ``10min``. Can be disabled by setting to ``0s``.
...
- **ton_pulse_delay**, **ton_delay**, **toff_pulse_delay**, **toff_delay** (*Optional*, :ref:`config-time`): These options are for advanced power management tuning and are only needed when using the ``power_pin``. For most supported models, these values are pre-configured. They are required when using the ``GENERIC`` model with a ``power_pin``, or if you need to override the default timings for your specific hardware.
...
- **debug** (*Optional*, boolean): Enables verbose logging for the underlying `esp_modem`_ library. Defaults to ``false``.

Automations:
------------

- **on_connect** (*Optional*, :ref:`Automation <automation>`): An action to be performed when the modem gets an IP address.
- **on_disconnect** (*Optional*, :ref:`Automation <automation>`): An action to be performed when the modem loses its IP address.
- **on_start_ppp** (*Optional*, :ref:`Automation <automation>`): An action to be performed when the modem starts the PPP connection.
- **on_enable** (*Optional*, :ref:`Automation <automation>`): An action to be performed when the modem is enabled.
- **on_not_responding** (*Optional*, :ref:`Automation <automation>`): An action to be performed when the modem is not responding to commands.

Actions
-------

.. _modem-disable_action:
...
.. _modem-connected_condition:

``modem.connected`` Condition
-----------------------------

This :ref:`Condition <config-condition>` checks if the modem is currently connected and has an IP address.
...
.. _modem-enabled_condition:

``modem.enabled`` Condition
---------------------------

This :ref:`Condition <config-condition>` checks if the modem is currently enabled.
...
From :ref:`lambdas <config-lambda>`, you can call several methods for advanced control.
...
- ``.modem_handler->dce``: Provides access to the DCE object from the underlying `esp_modem`_ library. See the `DCE methods <https://docs.espressif.com/projects/esp-protocols/esp_modem/docs/latest/cxx_api_docs.html#modem-commands>`_ for available commands.
