...
This release adds support for alpha-blending in images when using LVGL. To do that, the `use_transparency` option in :doc:`components/image` and related components was renamed to `transparency` and changed from a boolean to an enum.
Also `type` has become a mandatory option for image components. See the image documentation for details.
