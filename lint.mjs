...
function checkNewlines(fname, content) {
  for (const { line, col } of findAll(content, "\r")) {
    addError(
      ...
      line,
      col,
      "File contains Windows newline. Please set your editor to Unix newline mode.",
    );
    return; // Only report first occurrence
  }
}
...
async function checkInternalLinks(fname, content, anchorCache) {
  ...
  while ((match = linkPattern.exec(content)) !== null) {
    ...
    if (
      !linkUrl.startsWith("/") &&
      /\.(png|jpg|jpeg|gif|svg|webp|pdf|zip)$/i.test(linkUrl)
    ) {
      continue;
    }
...
function checkAutomationHeadings(fname, content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // e.g. ### `arm_away` Action
    const noDotMatch = line.match(
      /^#{2,4}\s+`([a-z_][a-z0-9_]*)`\s+(?:Action|Condition)s?\s*$/i,
    );
    if (noDotMatch) {
    ...
    if (
      line.match(
        /^#{2,4}\s+`[a-z_][a-z0-9_.]*\.[a-z_][a-z0-9_]*`\s+(?:action|condition)s?\s*$/,
      )
    ) {
      addError(
        fname,
...
async function main() {
  console.log(
    `${colors.cyan}Running ESPHome documentation linter...${colors.reset}\n`,
  );
  ...
  for (const [fname, gitMode] of gitFiles) {
    ...
    if (
      ignoreFolders.some(
        (folder) => fname.startsWith(folder) || fname.includes(`/${folder}`),
      )
    ) {
      continue;
    }
  ...
  if (errors.size > 0) {
    for (const [fname, errs] of [...errors.entries()].sort()) {
      console.log(
        `${colors.boldGreen}### File ${colors.bright}${fname}${colors.reset}\n`,
      );
    ...
    console.log(
      `${colors.boldRed}Found ${errors.size} file(s) with errors${colors.reset}`,
    );
