...
- **udp_id** (**Required**, [ID](/guides/configuration-types#config-id)): The ID of the UDP client to use for sending logs. May be omitted if only
  one UDP client is configured.
- **time_id** (**Required**, [ID](/guides/configuration-types#config-id)): The ID of the time client to use for time-stamping logs. May be omitted
