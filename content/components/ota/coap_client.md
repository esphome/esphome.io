---
description: "Instructions for setting up Over-The-Air (OTA) updates for ESPs to download firmwares remotely by CoAP."
title: "OTA Update via CoAP Client"
params:
  seo:
    description: Instructions for setting up Over-The-Air (OTA) updates for ESPs to download firmwares remotely by CoAP.
...
The OTA (Over The Air) via CoAP Client update component allows your devices to install updated firmware on their own.
...
Since the device functions as an CoAP(S) client, it can be on a foreign network or behind a firewall. 
...
{{< anchor "ota_coap_client-flash_action" >}}

## `ota.coap_client.flash` Action
...
on_...:
  then:
    - ota.coap_client.flash:
        md5_url: coap://example.com/firmware.md5
        url: coap://example.com/firmware.ota.bin
