---
description: "Instructions for setting up QMI8658 Accelerometer and Gyroscope sensors."
...
([datasheet](https://www.lcsc.com/datasheet/C2842151.pdf)) sensors with ESPHome. The [I²C Bus](#i2c) is required to be set up in your configuration for this
sensor to work.
...
{{< img src="qmi8658-full.jpg" alt="Image" caption="QMI8658 Accelerometer/Gyroscope Sensor." width="50.0%" class="align-center" >}}

{{< img src="qmi8658-ui.png" alt="Image" width="80.0%" class="align-center" >}}
...
{{< anchor "qmi8658-enable_wake_on_motion_action" >}}
...
- **id** (**Optional**, [ID](#config-id)): The ID of the qmi8658 sensor.

- **acceleration_odr** (**Optional**, enum, [templatable](#config-templatable)): One of Low Power ODR modes, defaults to `LOWPOWER_21HZ`.

- **blanking_time** (**Optional**, int, [templatable](#config-templatable)): Interrupt blanking time (in number of accelerometer samples), defaults to `0`.

- **initial_pin_state** (**Required**, int, [templatable](#config-templatable)): Initial interrupt pin state: `0` for `LOW`, `1` for `HIGH`.

- **interrupt_pin** (**Required**, enum, [templatable](#config-templatable)): Interrupt pin to trigger WoM: `INT1` or `INT2`.

- **threshold** (**Required**, int, [templatable](#config-templatable)): Absolute value (1-255) in mg to trigger interrupt pin. The smaller threshold - the less acceleration it is required to trigger WoM.
...
- {{< docref "template/" >}}
- {{< docref "ultrasonic/" >}}
- {{< apiref "qmi8658/qmi8658.h" "qmi8658/qmi8658.h" >}}
