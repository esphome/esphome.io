...
- **enable_serial** (*Optional*, boolean): Force-enable the Arduino `Serial` object (UART0) for use in lambdas or external libraries. By default, unused Serial objects are excluded from the build to save 32 bytes of RAM. Use the [UART component](/components/uart) instead when possible, as it works across all platforms. Defaults to automatic detection.
